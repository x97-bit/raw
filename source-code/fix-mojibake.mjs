...
walk('./server', (err, files) => {
      ...
      content = content.replace(regex, (match, quote, inner) => {
        ...
        if (/[\u0080-\u00FF]/.test(inner)) {
          try {
            ...
            if (/[\u0600-\u06FF]/.test(decoded)) {
              modified = true;
              return quote + decoded.replace(new RegExp(quote, 'g'), '\\' + quote) + quote;
            }
          } catch(e) {}
        }
        return match;
  ...
  const fieldConfigPath = path.resolve('./server/routes/field-customization/fieldConfig.ts');
  if (fs.existsSync(fieldConfigPath)) {
    ...
    content = content.replace(/import \{ safeWriteAuditLog \} from ['"]\.\.\/\.\.\/utils\/safeAuditLog['"];\s*/, '');
  ...
  const customFieldsPath = path.resolve('./server/routes/field-customization/customFields.ts');
  ...
  const customFieldValuesPath = path.resolve('./server/routes/field-customization/customFieldValues.ts');

export function normalizeFieldLabel(displayLabel, fallbackLabel) {
  const trimmedLabel = typeof displayLabel === 'string' ? displayLabel.trim() : '';
...
export function buildFieldConfigMap(configs = []) {
  ...
  configs.forEach((config) => {
    map[config.fieldKey] = {
      ...
      displayLabel: typeof config.displayLabel === 'string' ? config.displayLabel : null,
    };
  });
  return map;
}
...
export function getFieldLabel(configMap, fieldKey, fallbackLabel) {
  return normalizeFieldLabel(configMap?.[fieldKey]?.displayLabel, fallbackLabel);
}

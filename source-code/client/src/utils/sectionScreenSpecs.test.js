...
describe('sectionScreenSpecs', () => {
  ...
  it('uses the requested current statement order for Qaim', () => {
    const qaimStatementKeys = getSectionColumns('port-3', 'statement').map((column) => column.key);
  ...
  it('preserves section-specific invoice fields per port', () => {
    ...
    const mondhiriyaStatementKeys = getSectionTargetFields('port-2', 'statement').map((field) => field.key);
    const qaimInvoiceKeys = getSectionTargetFields('port-3', 'invoice').map((field) => field.key);
    const qaimStatementKeys = getSectionTargetFields('port-3', 'statement').map((field) => field.key);
  ...
  it('exposes Mondhiriya invoice-derived statement fields without forcing them visible by default', () => {
    const mondhiriyaStatementFields = getSectionTargetFields('port-2', 'statement');

    expect(mondhiriyaStatementFields.find((field) => field.key === 'account_name')?.defaultVisible).toBe(false);
    expect(mondhiriyaStatementFields.find((field) => field.key === 'syr_cus')?.defaultVisible).toBe(false);
    expect(mondhiriyaStatementFields.find((field) => field.key === 'trader_note')?.defaultVisible).toBe(false);
    expect(mondhiriyaStatementFields.find((field) => field.key === 'notes')?.defaultVisible).toBe(false);
    expect(mondhiriyaStatementFields.find((field) => field.key === 'cost_usd')?.defaultVisible).toBeUndefined();
  });
  ...
  it('normalizes shared labels while preserving payment reference wording', () => {
    ...
    expect(qaimStatementColumns.find((column) => column.key === 'amount_usd')?.label).toBe('المبلغ دولار');
    expect(qaimStatementColumns.find((column) => column.key === 'cost_usd')?.label).toBe('الكلفة دولار');
    expect(qaimPaymentFields.find((field) => field.key === 'ref_no')?.label).toBe('رقم سند القبض');
  });
});

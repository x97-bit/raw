...
function drawPageBackdrop(ctx, spec, branded, assets) {
  ...
  if (branded && assets?.header) {
    headerHeight = Math.round(
      drawImageFitWidth(ctx, assets.header, 0, 0, spec.width)
    );
  } else {
  ...
  if (branded && assets?.footer) {
    footerHeight = Math.round(
      (spec.width / assets.footer.width) * assets.footer.height
    );
    ctx.drawImage(
      assets.footer,
      0,
      spec.height - footerHeight,
      spec.width,
      footerHeight
    );
  } else {
  ...
  if (branded && assets?.footer) {
    footerHeight = Math.round(
      drawImageFitWidth(
        ...
        assets.footer,
        0,
        spec.height - (spec.width / assets.footer.width) * assets.footer.height,
        spec.width
      )
    );
  } else {
...
function drawInvoiceFooterOverlay(
  ctx,
  spec,
  footerHeight,
  pageNumber,
...
function drawSummaryCards(ctx, spec, x, y, width, summaryCards) {
  ...
  for (let row = 0; row < rows; row += 1) {
    const rowCards = summaryCards.slice(
      row * cardsPerRow,
      (row + 1) * cardsPerRow
    );
...
function drawTable(
  ...
  columns.forEach((column, index) => {
    ...
    drawRoundedRect(
      ...
      headerHeight,
      0,
      TAY_ALRAWI_BRAND_COLORS.tableNavy,
      "#d7dbe4",
      1
    );
  ...
  if (totalsRow) {
    ...
    columns.forEach((column, index) => {
      ...
      const value =
        index === 0
          ? "المجموع"
          : totalsRow[column.key] !== undefined
            ? formatCellValue(totalsRow[column.key], column.format)
            : "";
...
function computeAdaptiveColumnLayouts(
  columns,
  tableWidth,
  orientation = "landscape"
) {
  const compactScale =
    columns.length >= (orientation === "portrait" ? 7 : 9) ? 0.9 : 1;
  const baseWidths = columns.map(column => {
    ...
    if (
      ...
      return (isPortrait ? 170 : 215) * compactScale;
  ...
  if (totalBaseWidth <= tableWidth) {
    ...
    return baseWidths.map(
      (width, index) => width + (extraWidth * weights[index]) / totalWeight
    );
  }

  const minWidth = orientation === "portrait" ? 84 : 96;
  const scaledWidths = baseWidths.map(width =>
    Math.max(minWidth, (width / totalBaseWidth) * tableWidth)
  );
  ...
  const shrinkBudget = scaledWidths.reduce(
    (sum, width) => sum + Math.max(0, width - minWidth),
    0
  );
...
function paginateMeasuredRows(
  ...
  while (startIndex < rowHeights.length) {
    ...
    while (endIndex < rowHeights.length) {
      const reserveHeight =
        endIndex === rowHeights.length - 1 ? totalsHeight : 0;
      const nextHeight = rowHeights[endIndex];
      const wouldOverflow =
        usedHeight + nextHeight + reserveHeight > availableHeight;
...
function getAdaptiveTableMetrics(orientation, columnCount) {
  ...
  return {
    ...
    summaryValueLineHeight: orientation === "portrait" ? 100 : 121,
  };
}
...
function measureWrappedLineCount(
  ctx,
  text,
  { maxWidth, maxLines, size, weight }
) {
  setFont(ctx, weight, size);
  return getWrappedLines(ctx, text, maxWidth, maxLines).length;
}

function drawWrappedTableCellText(
  ctx,
  text,
  {
    x,
...
function fitColumnsForHeaders(ctx, columns, tableWidth, orientation, metrics) {
  ...
  const baseWidths = computeAdaptiveColumnLayouts(
    columns,
    tableWidth,
    orientation
  );
  ...
  while (fontSize >= minFontSize) {
    ...
    const targetWidths = columns.map((_, i) =>
      Math.max(baseWidths[i], requiredWidths[i])
    );
...
function measureAdaptiveTableLayout(
  ...
  const headerHeight = columns.reduce((maxHeight, column, index) => {
    ...
    return Math.max(
      maxHeight,
      metrics.paddingY * 2 + lineCount * metrics.headerLineHeight
    );
  }, metrics.minHeaderHeight);

  const rowHeights = rows.map(row => {
    if (row.isDailySummary) return metrics.minRowHeight - 16;

    return columns.reduce((maxHeight, column, index) => {
      const innerWidth = Math.max(
        18,
        columnWidths[index] - metrics.paddingX * 2
      );
      const value = formatExportCellValue(
        resolveExportColumnValue(row, column),
        column.format
      );
      const transTypeId = Number(row?.TransTypeID || row?.transTypeId || 0);
      const direction = String(
      ...
      return Math.max(
        maxHeight,
        metrics.paddingY * 2 + lineCount * metrics.cellLineHeight
      );
    ...
    ? columns.reduce((maxHeight, column, index) => {
        ...
        const value =
          index === 0
            ? "\u0627\u0644\u0645\u062c\u0645\u0648\u0639"
            : totalsRow[column.key] !== undefined
              ? formatExportCellValue(totalsRow[column.key], column.format)
              : "";
        const lineCount = measureWrappedLineCount(ctx, value, {
          maxWidth: innerWidth,
          maxLines: 1,
...
function drawAdaptiveSummaryCards(ctx, spec, x, y, width, summaryCards) {
  ...
  const metrics = getAdaptiveTableMetrics(
    spec.width > spec.height ? "landscape" : "portrait",
    summaryCards.length
  );
  const cardsPerRow = Math.min(
    summaryCards.length,
  ...
  for (let row = 0; row < rows; row += 1) {
    ...
    rowCards.forEach((card, index) => {
      ...
      const labelHeight = drawWrappedText(
        ctx,
        card.label,
        cardX + cardWidth - 20,
        cardY + 28,
...
function drawAdaptiveTable(
  ctx,
  { x, y, width, columns, rows, rowHeights, totalsRow, orientation, layout }
) {
  const tableLayout =
    layout ||
    measureAdaptiveTableLayout(ctx, {
      columns,
      rows,
      totalsRow,
      width,
  ...
  const { metrics, columnWidths, headerHeight, totalsHeight } = tableLayout;
  ...
  if (totalsRow) {
    ...
    columns.forEach((column, index) => {
      ...
      while (
        cellFontSize > 14 &&
        ctx.measureText(String(value ?? "")).width > innerWidth
      ) {
        cellFontSize -= 2;
        setFont(ctx, "700", cellFontSize);
      }
...
function drawSaudiStatementHeaderGrid(ctx, spec, startY, grid) {
  ...
  if (grid.accountName) {
    drawText(
      ctx,
      `${grid.accountLabel || "اسم التاجر"} : ${grid.accountName}`,
      rightColX,
      cursorY,
      {
        align: "right",
  ...
  if (grid.selectedLabel) {
    drawText(
      ctx,
      `${grid.selectedLabel}: ${grid.selectedValue || "---"}`,
      centerColX,
      cursorY,
      {
        align: "center",
...
function drawPartnershipHeaderGrid(ctx, spec, startY, grid) {
  ...
  drawText(
    ctx,
    `المبلغ عليه: ${grid.amountOnValue || "---"}`,
    rightColX,
    cursorY,
    {
      align: "right",
...
async function exportReportAsCanvasPdf({
  rows,
  columns,
  title,
  subtitle,
  ...
  const { headerHeight, footerHeight } = drawPageBackdrop(
    pageProbe.ctx,
    pageProbe.spec,
    branded,
    assets
  ...
  let firstPageTableY;
  if (usePartnerGrid) {
    const summaryEndY = drawPartnershipHeaderGrid(
      pageProbe.ctx,
      spec,
      headerHeight + 8,
      summaryGrid
    ...
    const summaryEndY = drawSaudiStatementHeaderGrid(
      pageProbe.ctx,
      spec,
      headerHeight + 8,
      summaryGrid
    ...
    const summaryEndY = drawTextLineSummary(
      pageProbe.ctx,
      spec,
      headerHeight + 8,
      summaryCards
    ...
    const titleEndY = drawCenteredReportHeader(
      pageProbe.ctx,
      spec,
      headerHeight + topGap,
      { title, subtitle }
    );
    const summaryEndY = drawAdaptiveSummaryCards(
      pageProbe.ctx,
      spec,
      marginX,
      titleEndY + 8,
      spec.width - marginX * 2,
      summaryCards
    );
    firstPageTableY = summaryCards.length
      ? summaryEndY + tableGap
      : titleEndY + 16;
  }
  ...
  const availableFirstPageHeight = Math.max(
    120,
    spec.height - footerHeight - 26 - firstPageTableY - tableLayout.headerHeight
  );
  const availableOtherPageHeight = Math.max(
    120,
    spec.height - footerHeight - 26 - nextPageTableY - tableLayout.headerHeight
  );
  ...
  for (let pageIndex = 0; pageIndex < pageSlices.length; pageIndex += 1) {
    ...
    } = drawPageBackdrop(ctx, spec, branded, assets);

    let tableY;
    if (usePartnerGrid) {
      if (pageIndex === 0) {
        const currentSummaryEndY = drawPartnershipHeaderGrid(
          ctx,
          spec,
          currentHeaderHeight + 8,
          summaryGrid
        );
      ...
      if (pageIndex === 0) {
        const currentSummaryEndY = drawSaudiStatementHeaderGrid(
          ctx,
          spec,
          currentHeaderHeight + 8,
          summaryGrid
      ...
      if (pageIndex === 0) {
        const currentSummaryEndY = drawTextLineSummary(
          ctx,
          spec,
          currentHeaderHeight + 8,
          summaryCards
      ...
      const currentTitleEndY = drawCenteredReportHeader(
        ctx,
        spec,
        currentHeaderHeight + topGap,
        { title, subtitle }
      );
      if (pageIndex === 0) {
        const currentSummaryEndY = drawAdaptiveSummaryCards(
          ctx,
          spec,
          marginX,
          currentTitleEndY + 8,
        ...
        tableY = summaryCards.length
          ? currentSummaryEndY + tableGap
          : currentTitleEndY + 16;
      } else {
        tableY = nextPageTableY;
      }
...
function buildSaudiStatementPdfSummaryCards(
  printContext = {},
...
function buildUnifiedSaudiStatementPdfSummaryCards(
  printContext = {},
  templateVariant = "both"
) {
  const totals = printContext?.totals || {};
...
function measureInvoiceSectionHeight(ctx, section, width) {
  ...
  if (isInvoiceNotesSection(section)) {
    const totalNotesHeight = (section.items || []).reduce(
      (sum, item, index) =>
        sum +
...
function drawInvoiceHeaderBlock(
  ...
  spec,
  assets,
  { title, companyName, transactionTypeLabel, metaItems },
  startY
) {
  ...
  const shouldShowSideType = Boolean(
    transactionTypeLabel && transactionTypeLabel !== titleText
  );
  ...
  metaItems.slice(0, 4).forEach((item, index) => {
    drawInvoiceMetaCard(
      ctx,
      shellX + metaInset + index * (metaWidth + metaGap),
      metaY,
      metaWidth,
      72,
      item.label,
...
function drawInvoiceContinuationHeader(
  ctx,
  spec,
  { title, referenceText },
  startY
...
function drawInvoiceSection(ctx, spec, section, startY, width) {
  ...
  if (isInvoiceNotesSection(section)) {
    (section.items || []).forEach(item => {
      const noteHeight = measureInvoiceNoteCardHeight(
        ctx,
        item.value,
        contentWidth - 28
      );
...
async function exportInvoiceAsCanvasPdfV2({
  ...
  const sections = buildInvoiceExportSections(
    transaction,
    sectionKey,
    targetKey
  );
  const assets = await loadBrandAssets(
    branded ? { footer: TAY_ALRAWI_BRAND_ASSETS.invoiceFooter } : {}
  );
  const pages = [];
  ...
  let currentY =
    drawInvoiceHeaderBlock(
      ...
      currentPage.headerHeight + 26
    ) + 18;

  sections.forEach(section => {
    const sectionHeight = measureInvoiceSectionHeight(
      currentPage.ctx,
      section,
      shellWidth
    );
    const availableBottom =
      currentPage.spec.height - currentPage.footerHeight - 28;

    if (currentY + sectionHeight > availableBottom) {
      pages.push(currentPage);
      currentPage = createInvoicePage();
      currentY =
        drawInvoiceContinuationHeader(
          currentPage.ctx,
          currentPage.spec,
          {
            title: documentTitle,
  ...
  pages.forEach((page, pageIndex) => {
    ...
    doc.addImage(
      page.canvas.toDataURL("image/png", 1),
      "PNG",
      0,
      0,
      page.spec.pdfWidth,
...
function getInvoiceFields(transaction, transactionTypeLabel) {
  return [
    ...
    {
      label: "العملة",
      value: getCurrencyLabel(transaction.CurrencyName || transaction.Currency),
    },
    { label: "اسم السائق", value: transaction.DriverName || "-" },
    {
      label: "رقم السيارة",
      value: transaction.PlateNumber || transaction.VehiclePlate || "-",
    },
    {
      label: "نوع البضاعة",
      value: transaction.GoodTypeName || transaction.GoodType || "-",
    },
    {
      label: "الوزن",
...
async function exportInvoiceAsCanvasPdf({
  transaction,
  title,
  companyName,
  branded,
...
function buildPortStatementSummaryCards(
  ...
  if (sectionKey === "port-1") {
    ...
    cards.push({ label: "مجموع الامتار", value: fmt(totalMeters) });
...
export async function exportInvoicePDF({
  ...
  const branded = shouldUseTayAlRawiBranding({
    sectionKey,
    portId,
    transaction,
  });

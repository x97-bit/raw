...
describe("buildOrderedFormSections", () => {
  it("keeps custom fields near their configured anchors instead of pushing them to the end", () => {
    ...
    expect(result[0].items.map(item => item.key)).toEqual([
      "ref_no",
      "trans_date",
      "custom_1",
    ]);
    expect(result[1].items.map(item => item.key)).toEqual([
      "amount_usd",
      "amount_iqd",
    ]);
  });
...
describe("filterSectionsByCurrency", () => {
  ...
  it("leaves non-currency-suffixed fields (trans_price, syr_cus) untouched", () => {
    ...
    expect(usdResult[0].items.map(i => i.key)).toEqual([
      "trans_price",
      "syr_cus",
      "amount_usd",
    ]);
    const iqdResult = filterSectionsByCurrency(sections, "IQD");
    expect(iqdResult[0].items.map(i => i.key)).toEqual([
      "trans_price",
      "syr_cus",
      "amount_iqd",
    ]);

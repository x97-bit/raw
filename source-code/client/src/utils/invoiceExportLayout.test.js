...
describe("invoiceExportLayout", () => {
  it("formats invoice field values for the export cards", () => {
    expect(resolveInvoiceFieldValue(SAMPLE_TRANSACTION, "currency")).toBe(
      getCurrencyLabel("USD")
    );
    expect(resolveInvoiceFieldValue(SAMPLE_TRANSACTION, "amount_usd")).toBe(
      "$1,500"
    );
    expect(resolveInvoiceFieldValue(SAMPLE_TRANSACTION, "notes")).toBe(
      "ملاحظة داخلية"
    );
  });
  ...
  it("uses transport-specific reference labels when exporting transport documents", () => {
    expect(
      buildInvoiceHeaderMeta(SAMPLE_TRANSACTION, "invoice", "transport-1")[0]
        .label
    ).toBe("رقم استحقاق النقل");
    expect(
      buildInvoiceHeaderMeta(
        { ...SAMPLE_TRANSACTION, TransTypeID: 2 },
        "payment",
        "transport-1"
      )[0].label
    ).toBe("رقم سند الدفع");
  ...
  it("uses the section invoice layout instead of the generic field list", () => {
    ...
    const flattenedKeys = sections.flatMap(section =>

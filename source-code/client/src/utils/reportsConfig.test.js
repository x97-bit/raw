...
describe("reportsConfig", () => {
  it("exposes the supported report ports", () => {
    expect(REPORT_PORTS.map(port => port.id)).toEqual([
      "port-1",
      "port-2",
      "port-3",
    ]);
    expect(
      REPORT_PORTS.every(
        port => Array.isArray(port.iconLines) && port.iconLines.length > 0
      )
    ).toBe(true);
  });
  ...
  it("builds export columns and summary cards for reports", () => {
    expect(
      EXPENSES_EXPORT_COLUMNS.some(column => column.key === "chargeTarget")
    ).toBe(true);
    expect(
      EXPENSES_EXPORT_COLUMNS.some(column => column.key === "accountName")
    ).toBe(true);
    expect(
      PROFITS_EXPORT_COLUMNS.some(column => column.key === "ProfitUSD")
    ).toBe(true);
    expect(
      PROFITS_BY_TRADER_EXPORT_COLUMNS.some(
        column => column.key === "totalProfitIQD"
      )
    ).toBe(true);

...
const createDebitNoteSpec = () => ({
  ...
  layout: [
    ...
    createFormSection('بيانات الشحنة', ['driver_name', 'vehicle_plate', 'good_type', 'gov_name', 'weight']),
...
const RAW_SECTION_SCREEN_SPECS = {
  'transport-1': {
    ...
    list: {
      columns: [
        ...
        createColumn('direction', 'TransTypeName', 'نوع الحركة', 'badge'),
        createColumn('trans_date', 'TransDate', 'التاريخ', 'date'),
        createColumn('carrier_name', 'CarrierName', 'اسم الناقل'),
        createColumn('currency', 'Currency', 'العملة', 'currency'),
        createColumn('amount_usd', 'AmountUSD', 'المبلغ دولار', 'money_usd_bold'),
        createColumn('amount_iqd', 'AmountIQD', 'المبلغ دينار', 'money_iqd_bold'),
    ...
    invoice: {
      fields: [
        ...
        createField('trans_date', 'التاريخ', 'date'),
        createField('carrier_name', 'اسم الناقل', 'text'),
        createField('currency', 'العملة', 'text'),
        createField('account_name', 'اسم التاجر', 'text'),
      ...
      layout: [
        createFormSection('المعلومات الأساسية', ['ref_no', 'trans_date', 'carrier_name', 'currency', 'account_name']),
        createFormSection('تفاصيل الحركة', ['good_type', 'car_qty', 'gov_name']),
        createFormSection('القيم المالية', ['amount_usd', 'amount_iqd']),
    ...
    payment: {
      ...
      layout: [
        createFormSection('المعلومات الأساسية', ['ref_no', 'trans_date', 'currency', 'account_name']),
        createFormSection('القيم المالية', ['amount_usd', 'amount_iqd']),
        createFormSection('الملاحظات', ['notes']),
      ],
    },
  ...
  'partnership-1': {
    ...
    invoice: {
      ...
      layout: [
        createFormSection('المعلومات الأساسية', ['ref_no', 'trans_date', 'account_name', 'currency']),
        createFormSection('بيانات الشحنة', ['good_type', 'vehicle_plate', 'driver_name']),
...
export function getSectionFieldLabelMap(sectionKey, targetKey) {
  return Object.fromEntries(
    getSectionTargetFields(sectionKey, targetKey).map((field) => [field.key, field.label]),
  );
}

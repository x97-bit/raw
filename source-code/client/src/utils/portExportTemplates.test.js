...
import { buildListExportTemplates, buildStatementExportTemplates } from './portExportTemplates';
...
describe('portExportTemplates', () => {
  it('builds list templates with port-aware titles and filenames', () => {
    ...
    expect(templates[1].title).toBe(
      'منفذ السعودية - تشغيلي',
    );
    expect(templates[1].filename).toBe(
      'منفذ السعودية_تشغيلي',
    );
  });
  ...
  it('builds statement templates with account-aware titles and filenames', () => {
    ...
    expect(templates[0].label).toBe('النموذج الحالي');
    expect(templates[1].title).toContain(
      'كشف حساب - أبو حسن بعقوبة',
    );
    expect(templates[1].filename).toContain(
      'كشف_حساب_أبو حسن بعقوبة',
    );
  });

  it('falls back to template-only names when account is missing', () => {
    const templates = buildStatementExportTemplates('port-1', [{ key: 'AmountUSD', label: 'x' }], '');

    expect(templates[1].title).toBe(
      'كشف حساب - نموذج دولار',
    );

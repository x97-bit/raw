...
describe("transactionTypeLabels", () => {
  ...
  it("supports custom debit-note labels and references", () => {
    expect(getTransactionTypeLabel("", 3, { recordType: "debit-note" })).toBe(
      "سند إضافة"
    );
    expect(getTransactionReferenceLabel(3, { recordType: "debit-note" })).toBe(
      "رقم سند الإضافة"
    );
  });
  ...
  it("switches transport labels and references when the section is transport", () => {
    ...
    expect(getTransactionTypeLabel("", 1, { sectionKey: "transport-1" })).toBe(
      "استحقاق نقل"
    );
    expect(getTransactionTypeLabel("", 2, { sectionKey: "transport-1" })).toBe(
      "سند دفع"
    );
    expect(getTransactionReferenceLabel(1, { sectionKey: "transport-1" })).toBe(
      "رقم استحقاق النقل"
    );
    expect(getTransactionReferenceLabel(2, { sectionKey: "transport-1" })).toBe(
      "رقم سند الدفع"
    );
  });
});

...
describe("sectionExportProfiles", () => {
  it("provides extra list templates for configured port sections", () => {
    ...
    expect(
      profiles[0].columns.some(column => column.key === "DriverName")
    ).toBe(true);
    expect(profiles[1].columns.some(column => column.key === "FeeUSD")).toBe(
      true
    );
    expect(
      profiles[1].columns.some(column => column.key === "TransPrice")
    ).toBe(true);
  });
  ...
  it("provides the four Saudi statement print templates with the expected money columns", () => {
    ...
    expect(profiles[0].columns.some(column => column.key === "AmountUSD")).toBe(
      true
    );
    expect(
      profiles[0].columns.find(column => column.key === "AmountUSD")?.label
    ).toBe("المبلغ $");
    expect(profiles[1].columns.some(column => column.key === "AmountIQD")).toBe(
      true
    );
    expect(profiles[2].columns.some(column => column.key === "AmountUSD")).toBe(
      true
    );
    expect(profiles[2].columns.some(column => column.key === "AmountIQD")).toBe(
      true
    );
    expect(
      profiles[3].columns.some(column => column.key === "Governorate")
    ).toBe(true);
  });
  ...
  it("provides a Mondhiriya USD statement template with the requested columns", () => {
    ...
    const usdProfile = profiles.find(
      profile => profile.id === "mondhiriya-usd"
    );
    ...
    expect(
      usdProfile?.columns.find(column => column.key === "SyrCus")?.label
    ).toBe("الكمرك السوري");
  });
  ...
  it("uses the Saudi statement templates for Qaim printing", () => {
    ...
    expect(
      profiles.every(profile =>
        profile.columns.every(column => column.key !== "Meters")
      )
    ).toBe(true);

...
const normalizeVisibleFieldKeys = (visibleFieldKeys) => (
  visibleFieldKeys instanceof Set ? visibleFieldKeys : new Set(visibleFieldKeys || [])
);
...
export function applyDefaultsToTransactionDraft(draft = {}, defaults = {}, visibleFieldKeys = []) {
  ...
  if (canFillField('driver_name') && !hasValue(next._driverText) && hasValue(defaults.DriverName) && (!hasValue(next.DriverID) || next.DriverID === defaults.DriverID)) {
    next._driverText = defaults.DriverName;
  }
  if (canFillField('vehicle_plate') && !hasValue(next._vehicleText) && hasValue(defaults.VehiclePlate) && (!hasValue(next.VehicleID) || next.VehicleID === defaults.VehicleID)) {
    next._vehicleText = defaults.VehiclePlate;
  }
  if (canFillField('good_type') && !hasValue(next._goodText) && hasValue(defaults.GoodTypeName) && (!hasValue(next.GoodTypeID) || next.GoodTypeID === defaults.GoodTypeID)) {
    next._goodText = defaults.GoodTypeName;
  }
  if (canFillField('gov_name') && !hasValue(next._govText) && hasValue(defaults.GovName) && (!hasValue(next.GovID) || next.GovID === defaults.GovID)) {
    next._govText = defaults.GovName;
  }
  if (canFillField('company_name') && !hasValue(next._companyText) && hasValue(defaults.CompanyName) && (!hasValue(next.CompanyID) || next.CompanyID === defaults.CompanyID)) {
    next._companyText = defaults.CompanyName;
  }
  if (canFillField('carrier_name') && !hasValue(next._carrierText) && hasValue(defaults.CarrierName) && (!hasValue(next.CarrierID) || next.CarrierID === defaults.CarrierID)) {

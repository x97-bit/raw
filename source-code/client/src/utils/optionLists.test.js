...
import { normalizeAutocompleteOptions, normalizeStringOptions } from './optionLists';

describe('normalizeStringOptions', () => {
  it('removes blanks and duplicates while preserving order', () => {
    expect(normalizeStringOptions(['', ' نعم ', 'لا', 'نعم', null, 'لا', '  '])).toEqual(['نعم', 'لا']);
  });
});
...
describe('normalizeAutocompleteOptions', () => {
  it('removes empty rows and duplicate id-label pairs', () => {
    ...
    expect(options.map((entry) => entry.reactKey)).toEqual(['1::أحمد', '2::محمد', '::محمد']);
  });
});

...
async function generateInvoiceHtmlLegacy(transaction = {}, { sectionKey, portId } = {}) {
  ...
  tableRows.forEach(row => {
    if (row.currency === "دولار" && row.amount) {
      const num = Number(String(row.amount).replace(/[^\d.-]/g, ""));
...
<div class="page">
  ...
  <div class="content">
    ...
    <table class="invoice-table">
      <thead><tr><th style="width:8%">#</th><th style="width:32%">الوصف</th><th style="width:22%">الملاحظات</th><th style="width:13%">العملة</th><th style="width:25%">المبلغ</th></tr></thead>
      <tbody>${tableRows.map((row, idx) => `<tr><td style="font-weight:700;color:#64748b;">${idx + 1}</td><td style="text-align:right;padding-right:16px;">${escapeHtml(row.description)}</td><td style="font-size:11px;color:#64748b;">${escapeHtml(row.notes)}</td><td class="currency-cell">${escapeHtml(row.currency)}</td><td class="amount-cell">${escapeHtml(row.amount)}</td></tr>`).join("")}</tbody>
      ${showTotal ? `<tfoot><tr style="background:${navy};"><td colspan="3" style="text-align:right;padding:10px 16px;font-weight:800;color:#ffffff;font-size:12px;">الإجمالي</td><td colspan="2" style="text-align:center;padding:10px 14px;font-weight:800;color:#ffffff;font-size:14px;direction:ltr;font-family:'Inter','Segoe UI',monospace;">${totalDisplay.join(" + ")}</td></tr></tfoot>` : ""}
    </table>

...
export const buildTransactionDetailItems = ({
  ...
  const costUsd = transaction.CostUSD
    ...
    : "-";
  const positiveBalanceColor = isTransport
    ? "text-rose-600"
    : "text-emerald-600";
  ...
  return [
    { label: "التاريخ", value: transaction.TransDate?.split("T")[0].split(" ")[0] || "-" },
    {
      ...
      badge: true,
      type: transaction.TransTypeID,
    },
    {
      label: "اسم التاجر",
      value: transaction.AccountName || transaction.TraderName || "-",
      bold: true,
    },
    {
      label: getTransactionReferenceLabel(transaction.TransTypeID, {
        sectionKey,
    ...
    {
      label: "الكلفة دينار",
      value: transaction.CostIQD
        ? formatTransactionModalNumber(transaction.CostIQD)
        : "-",
    },
    {
      label: "نوع البضاعة",
    ...
    },
    {
      ...
      value: transaction.Weight
        ? formatTransactionModalNumber(transaction.Weight)
        : "-",
    },
    ...
    ...(transaction.SyrCus
      ? [
          {
            label: "الكمرك السوري",
            value: `${formatTransactionModalNumber(transaction.SyrCus)}`,
          },
        ]
      : []),
    ...(transaction.CarQty
      ? [{ label: "عدد السيارات", value: transaction.CarQty }]
      : []),
    ...(transaction.TransPrice
      ? [
          {
            label: "سعر النقل",
            value: formatTransactionModalNumber(transaction.TransPrice),
          },
        ]
      : []),
    ...(transaction.CarrierName
      ? [{ label: "اسم الناقل", value: transaction.CarrierName }]
      : []),

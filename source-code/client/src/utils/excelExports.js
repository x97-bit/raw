...
function computeColumnWidth(column, values = []) {
  ...
  return Math.min(
    40,
    Math.max(column?.width || 0, minimumWidth, preferredWidth)
  );
}
...
export function exportToExcel(rows, columns, filename, sheetTitle) {
  ...
  const data = rows.map(row =>
    columns.map(column => {
      ...
      if (
        (column.format === "money" || column.format === "money_iqd") &&
        value !== undefined &&
        value !== null &&
        value !== ""
      )

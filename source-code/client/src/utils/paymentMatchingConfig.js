...
export function buildPaymentDashboardStats(dashboard) {
  ...
  if (Array.isArray(shipmentStats)) {
    return {
      paid: shipmentStats.find((entry) => entry.payment_status === 'paid') || { count: 0, total_usd: 0, total_iqd: 0 },
      partial: shipmentStats.find((entry) => entry.payment_status === 'partial') || { count: 0, remaining_usd: 0, remaining_iqd: 0 },
      unpaid: shipmentStats.find((entry) => entry.payment_status === 'unpaid') || { count: 0, remaining_usd: 0, remaining_iqd: 0 },
  ...
  return {
    ...
    partial: { count: shipmentStats?.partial || 0, remaining_usd: 0, remaining_iqd: 0 },
    unpaid: { count: shipmentStats?.unmatched || 0, remaining_usd: 0, remaining_iqd: 0 },
    payments: dashboard.paymentStats || { unallocated_usd: 0, unallocated_iqd: 0 },
  };
}

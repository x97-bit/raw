...
export const DEBTOR_CONFIGS = {
  basim: {
    label: 'باسم',
    columns: [
      { key: 'trans_date', dataKey: 'TransDate', label: 'التاريخ', render: (value) => value?.split(' ')[0] || '-' },
      { key: 'amount_usd', dataKey: 'AmountUSD', label: 'المبلغ ($)', render: (value) => (value ? `$${formatNum(value)}` : '-'), bold: true, color: true },
      { key: 'fee_usd', dataKey: 'FeeUSD', label: 'الرسوم ($)', render: (value) => (value ? `$${formatNum(value)}` : '-') },
      { key: 'amount_iqd', dataKey: 'AmountIQD', label: 'المبلغ (د.ع)', render: (value) => (value ? formatNum(value) : '-') },
      { key: 'fee_iqd', dataKey: 'FeeIQD', label: 'الرسوم (د.ع)', render: (value) => (value ? formatNum(value) : '-') },
      { key: 'trans_type', dataKey: 'TransType', label: 'نوع الحركة', render: (value) => value || '-' },
      { key: 'state', dataKey: 'State', label: 'الحالة', render: (value) => value || '-' },
  ...
  luay: {
    ...
    columns: [
      ...
      { key: 'good_type', dataKey: 'GoodTypeName', label: 'نوع البضاعة', render: (value) => value || '-' },
      { key: 'weight', dataKey: 'Weight', label: 'الوزن', render: (value) => (value ? formatNum(value) : '-') },
      { key: 'amount_usd', dataKey: 'AmountUSD', label: 'المبلغ ($)', render: (value) => (value ? `$${formatNum(value)}` : '-'), bold: true, color: true },
      { key: 'notes', dataKey: 'Notes', label: 'ملاحظات', render: (value) => value || '', isNotes: true },
    ],
    formFields: [
  ...
  abdalkarem: {
    ...
    columns: [
      ...
      { key: 'amount_usd', dataKey: 'AmountUSD', label: 'المبلغ ($)', render: (value) => (value ? `$${formatNum(value)}` : '-'), bold: true, color: true },
      { key: 'amount_iqd', dataKey: 'AmountIQD', label: 'المبلغ (د.ع)', render: (value) => (value ? formatNum(value) : '-') },
      { key: 'account_name', dataKey: 'AccountName', label: 'اسم الحساب', render: (value) => value || '-' },
      { key: 'notes', dataKey: 'Notes', label: 'ملاحظات', render: (value) => value || '', isNotes: true },
    ],
    formFields: [
      { key: 'TransDate', label: 'التاريخ', type: 'date' },
...
export const DEFAULT_DEBT_COLUMNS = [
  ...
  { key: 'amount_usd', dataKey: 'AmountUSD', label: 'المبلغ ($)', render: (value) => (value ? `$${formatNum(value)}` : '-'), bold: true, color: true },
  { key: 'amount_iqd', dataKey: 'AmountIQD', label: 'المبلغ (د.ع)', render: (value) => (value ? formatNum(value) : '-') },
  { key: 'remaining_usd', dataKey: 'RemainingUSD', label: 'المتبقي ($)', render: (value) => (value ? `$${formatNum(value)}` : '-') },
  { key: 'state', dataKey: 'State', label: 'الحالة', render: (value) => value || '-' },
  { key: 'notes', dataKey: 'Notes', label: 'ملاحظات', render: (value) => value || '', isNotes: true },
];
...
export function getDebtorKey(accountName) {
  ...
  if (normalized.includes('نعمان') || normalized.includes('نومان')) return 'noman';
  if (normalized.includes('لؤي 2') || normalized.includes('لؤي2')) return 'luay2';
  if (normalized.includes('لؤي')) return 'luay';
  if (normalized.includes('عبد الكريم') || normalized.includes('عبدالكريم')) return 'abdalkarem';
  return null;
}
...
export function buildDebtSummaryRows(rows) {
  ...
  return [...summaryMap.values()].sort((a, b) => Math.abs(b.remainingUSD) - Math.abs(a.remainingUSD));
}
...
export function getInitialDebtFormState(debt = null) {
  return {
    ...
    TransDate: debt?.TransDate?.split(' ')[0] || new Date().toISOString().split('T')[0],

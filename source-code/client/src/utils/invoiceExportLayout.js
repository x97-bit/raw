...
import { getSectionFormLayout, getSectionTargetFields } from './sectionScreenSpecs';
import { getTransactionReferenceLabel, getTransactionTypeLabel } from './transactionTypeLabels';

const FALLBACK_LAYOUTS = {
  invoice: [
    { title: 'المعلومات الأساسية', keys: ['ref_no', 'trans_date', 'account_name', 'currency'] },
    { title: 'تفاصيل الشحنة', keys: ['driver_name', 'vehicle_plate', 'good_type', 'weight', 'qty', 'meters'] },
    { title: 'القيم المالية', keys: ['cost_usd', 'amount_usd', 'cost_iqd', 'amount_iqd'] },
    { title: 'تفاصيل إضافية', keys: ['gov_name', 'company_name', 'fee_usd', 'trans_price', 'carrier_name', 'car_qty', 'syr_cus'] },
...
export function buildInvoiceExportSections(transaction = {}, sectionKey, targetKey = 'invoice') {
  ...
  return activeLayout.map((section, sectionIndex) => ({
    key: section.key || `${targetKey}-section-${sectionIndex + 1}`,
    title: section.title || '',
    subtitle: section.subtitle || '',
    items: (section.keys || [])
      .map((fieldKey) => {
        const field = fieldMap.get(fieldKey) || { key: fieldKey, label: fieldKey, type: 'text' };
        return {
          key: fieldKey,

...
export const FIELD_CONFIG_TARGETS = {
  ...
  'debit-note': { key: 'debit-note', label: 'سند إضافة', shortLabel: 'سند إضافة' },
};
...
export function getAvailableFieldConfigTargets(sectionKey) {
  ...
  if (sectionKey === 'fx' || sectionKey.startsWith('fx-')) {
    return [
      FIELD_CONFIG_TARGETS.list,
      FIELD_CONFIG_TARGETS.invoice,
    ];
  }
...
export function usesLegacyFieldConfigFallback(targetKey) {
  return targetKey === 'default' || targetKey === 'list' || targetKey === 'statement';
}
...
export function isSectionTargetCompatible(sectionKey, targetKey) {
  ...
  return getAvailableFieldConfigTargets(sectionKey).some((target) => target.key === (targetKey || 'default'));
}
...
export function expandFieldSectionsForTarget(sectionKeys, targetKey) {
  ...
  return baseSections.map((sectionKey) => buildFieldConfigKey(sectionKey, targetKey));
}

export function matchesFieldTarget(field, sectionKey, targetKey) {
  const fieldSections = Array.isArray(field?.sectionKeys) ? field.sectionKeys : [];
  return getScopedFieldSectionKeys(sectionKey, targetKey).some((key) => fieldSections.includes(key));
}

...
const normalizeExportColumns = (columns = [], variant = "compact-export") =>
  columns.map(column => ({
    ...column,
...
const EXPORT_LABELS = {
  ...
  totalWeight:
    "\u0645\u062c\u0645\u0648\u0639 \u0627\u0644\u0648\u0632\u0646 \u0627\u0644\u0643\u0644\u064a",
  totalMeters: "مجموع الامتار الكلي",
};

const buildCombinedNotesColumn = () =>
  createDirectColumn("__combinedNotes", EXPORT_LABELS.notes, undefined, {
...
const buildSaudiStatementProfiles = ({
  includeSummaryCards = false,
  includeMeters = true,
} = {}) => {
  const summaryCardConfig = includeSummaryCards
    ? {
        summaryCards: ({ printContext }) =>
          buildSaudiStatementSummaryCards(printContext),
      }
    : {};
  const metersColumn = includeMeters
  ...
  return (columnKeys || [])
    .map(columnKey => {
      ...
      return {
        key: column.dataKey,
        label:
          columnKey === "ref_no" && sectionKey === "transport-1"
            ? "رقم المستند"
...
export function buildCurrentTemplateColumns(sectionKey, fallbackColumns = []) {
  const columnLibrary = getSectionColumnLibrary(sectionKey);
  const fallbackByExportKey = new Map(
    (fallbackColumns || []).map(column => [column.key, column])
  );

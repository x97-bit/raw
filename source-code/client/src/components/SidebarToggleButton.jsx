...
export default function SidebarToggleButton({ compact = false }) {
  const { hasSidebar, isDesktop, sidebarCollapsed, toggleSidebar } =
    useAppShell();
  ...
  const { label, title, Icon } = resolveButtonState({
    isDesktop,
    sidebarCollapsed,
  });
  ...
  return (
    ...
    >
      <span
        ...
        style={{
          background:
            "linear-gradient(90deg, transparent 0%, var(--sidebar-toggle-accent) 50%, transparent 100%)",
        }}
      />

      <span
        className={`relative z-10 flex items-center ${compact ? "justify-center" : "gap-2.5"}`}
      >

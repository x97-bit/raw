...
export default function AutocompleteInput({
  ...
  const filtered = useMemo(
    () =>
      normalizedOptions.filter(
        ({ label }) =>
          !normalizedText || label.toLowerCase().includes(normalizedText)
      ),
  ...
  const showDropdown =
    open &&
    !disabled &&
    (visibleOptions.length > 0 || showAddNew || normalizedText.length > 0);
  ...
  const updateDropdownPosition = useCallback(() => {
    ...
    let shouldOpenUpward =
      availableBelow < minimumComfortableSpace &&
  ...
  const dropdownContent =
    showDropdown && dropdownStyle ? (
      ...
      >
        <div className="flex items-center justify-between border-b border-utility-panel-border bg-utility-soft-bg px-4 py-2 text-[11px] font-semibold text-utility-muted">
          <span>{filtered.length} نتيجة</span>
          <span className="truncate pr-3">
            {text ? `بحث: ${text}` : "ابدأ بالكتابة"}
          </span>
        </div>
        ...
        {hiddenCount > 0 && (
          <div className="border-t border-utility-panel-border bg-utility-soft-bg px-4 py-2 text-center text-[11px] font-semibold text-utility-muted">

...
describe("stackNavigation", () => {
  ...
  it("resolves direct main-page destinations", () => {
    ...
    expect(resolveMainPageNavigation("field-management")).toEqual({
      page: "field-management",
    });
  });

  it("resolves section destinations from the shared catalog", () => {
    expect(resolveMainPageNavigation("port-1")).toEqual({
      page: "section",
      sectionId: "port-1",
    });
    expect(resolveMainPageNavigation("transport")).toEqual({
      page: "section",
      sectionId: "transport",
    });
  });
  ...
  it("returns null for unknown destinations", () => {
    ...
    expect(
      resolveSectionActionNavigation("unknown-page", "invoice")
    ).toBeNull();
  });

...
export function formatOutstandingAmount(usd, iqd) {
  const usdPart = Number(usd || 0) > 0 ? `$${formatPaymentMatchingNumber(usd)}` : '';
  const iqdPart = Number(iqd || 0) > 0 ? `${formatPaymentMatchingNumber(iqd)} د.ع` : '';
...
export function formatAllocationAmount(allocation = {}) {
  return formatOutstandingAmount(allocation.allocated_usd, allocation.allocated_iqd);
}

export function buildPaymentProgress(stats) {
  const total = (stats?.paid?.count || 0) + (stats?.partial?.count || 0) + (stats?.unpaid?.count || 0);
  const paidPct = total ? Math.round(((stats?.paid?.count || 0) / total) * 100) : 0;
  const partialPct = total ? Math.round(((stats?.partial?.count || 0) / total) * 100) : 0;

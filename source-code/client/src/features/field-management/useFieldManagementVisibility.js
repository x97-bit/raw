...
export default function useFieldManagementVisibility({
    ...
    () =>
      SECTIONS.filter(
        section =>
          ...
          isSectionTargetCompatible(section.key, selectedTarget)
      ),
    [selectedSection, selectedTarget]
  );

  const loadFieldConfigSet = useCallback(
    async (sectionKey, targetKey) => {
      const configKey = buildFieldConfigKey(sectionKey, targetKey);
      const configs = await api(`/field-config/${configKey}`);
      if (configs?.length > 0) return configs;
      if (
        usesLegacyFieldConfigFallback(targetKey) &&
  ...
  const loadFieldConfigs = useCallback(async () => {
    ...
    try {
      ...
      const sectionFields = getSectionFieldsForTarget(
        selectedSection,
        selectedTarget
      );
  ...
  useEffect(() => {
    setTargetSections(prev =>
      filterCompatibleSelection(prev, compatibleTargetSections)
    );
  ...
  const saveDisplayLabelEdit = useCallback(
    field => {
  ...
  const handleDragEnd = useCallback(() => {
    if (
      ...
      setFieldConfigs(prev =>
        reorderFieldConfigs(prev, dragIndex, dragOverIndex)
      );
    }
    setDragIndex(null);
  ...
  const moveTouchItem = useCallback(
    (fromIndex, toIndex) => {
  ...
  const selectAllTargetSections = useCallback(() => {
    setTargetSections(prev =>
      toggleAllSections(prev, compatibleTargetSections)
    );
  }, [compatibleTargetSections]);
  ...
  const copySettingsToSections = useCallback(async () => {
    ...
    try {
      ...
      for (const sectionKey of targetSections) {
        try {
          await api(
            `/field-config/${buildFieldConfigKey(sectionKey, selectedTarget)}`,
            {
              method: "PUT",
              body: JSON.stringify({ fields }),
            }
      ...
      showMessage(
        `تم نسخ الإعدادات إلى ${successCount} من ${targetSections.length} أقسام بنجاح`
      );
  ...
  const importFromSection = useCallback(
    async sourceKey => {
      setSaving(true);
      try {
        const sourceConfigs = await loadFieldConfigSet(
          sourceKey,
          selectedTarget
        );
        ...
        const sourceSectionLabel = SECTIONS.find(
          section => section.key === sourceKey
        )?.label;

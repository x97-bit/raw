...
describe('fieldManagementPageHelpers', () => {
  it('filters and restores selections safely', () => {
    ...
    expect(filterCompatibleSelection(['port-1', 'reports'], compatible)).toEqual(['port-1']);
    expect(ensureSelectionOrFallback([], 'list')).toEqual(['list']);
    expect(ensureSelectionOrFallback(['statement'], 'list')).toEqual(['statement']);
  });
  ...
  it('toggles multi selections and non-empty target selections', () => {
    ...
    expect(toggleNonEmptyTargetSelection(['invoice'], 'invoice', 'payment')).toEqual(['invoice']);
    expect(toggleNonEmptyTargetSelection(['invoice'], 'payment', 'invoice')).toEqual(['invoice', 'payment']);
  });
  ...
  it('merges base fields with scoped custom fields and configs', () => {
    const merged = mergeFieldManagementFieldConfigs({
      configs: [{ fieldKey: 'amount', visible: 1, sortOrder: 2, displayLabel: 'المبلغ النهائي' }],
      customFieldsList: [
        {
          id: 1,
    ...
    expect(merged.map((field) => field.key)).toEqual(['name', 'amount', 'custom_total']);
    expect(merged.find((field) => field.key === 'amount')?.displayLabel).toBe('المبلغ النهائي');
    expect(merged.find((field) => field.key === 'custom_total')?.visible).toBe(false);
  });
});

    ...
    <div className="surface-card rounded-xl shadow-sm border border-white/[0.06] overflow-hidden p-0">
      <div className="px-4 py-3 bg-white/[0.03] border-b border-white/[0.06]">
        <h3 className="font-bold text-[#eef3f7]">
          الحقول المخصصة - {currentConfigLabel}
        </h3>
      </div>
      ...
      {currentCustomFields.length === 0 ? (
        <div className="p-8 text-center text-[#91a0ad]">
          <PlusCircle
            size={40}
            className="mx-auto mb-3 opacity-30 text-[#64727f]"
          />
            ...
            <div key={field.id} className="px-4 py-3">
              {editingFieldId === field.id ? (
                <div className="space-y-3">
                  <div>
                    <label className="block text-xs font-medium text-[#c8d4df] mb-1">
                      اسم الحقل
                    </label>
                    <input
                      type="text"
                      ...
                      onKeyDown={event => {
                        if (event.key === "Enter" && !editingFormula)
                          onSaveEditField(field.id);
                  ...
                  {editingAvailableTargets.length > 1 && (
                    <div className="space-y-2">
                      <div className="flex items-center justify-between gap-3">
                        <label className="block text-xs font-medium text-[#c8d4df]">
                          الشاشات المستهدفة
                        </label>
                        <span className="text-xs text-[#91a0ad]">
                          يمكن اختيار أكثر من شاشة
                        </span>
                      </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      >
                        ...
                        editingCompatibleSections.length
                          ? "إلغاء الكل"
                          : "تحديد الكل"}
                      </button>
                    </div>
                  ...
                  {editingFormula && (
                    <div className="space-y-3">
                      ...
                      <div className="bg-white/[0.02] rounded-xl p-4 border border-white/[0.06]">
                              ...
                              {part.type === "operator" ? (
                                ...
                                <div className="flex items-center gap-2 w-full">
                                  ...
                                  >
                                    ...
                                    {getEditFormulaFields().map(
                                      formulaField => (
                                        <option
                                          key={formulaField.key}
                                          value={formulaField.key}
                                        >
                                          {formulaField.label}
                                        </option>
                                      )
                                  ...
                                  {editingFormula.formulaParts.filter(
                                    formulaPart => formulaPart.type === "field"
                                  ).length > 1 && (
                                    <button
                                      type="button"
                                      onClick={() =>
                                        onRemoveEditFormulaPart(index)
                                      }
                        ...
                        {editingFormula.formulaParts.some(
                ...
                <div className="flex items-center gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-[#eef3f7]">
                      {field.label}
                    </div>
                    <div className="text-xs text-[#91a0ad] mt-0.5 flex flex-wrap gap-2">
                      <span
                        className={`px-2 py-0.5 rounded ${field.fieldType === "formula" ? "bg-[#d6b36b]/[0.16] text-[#f6e7c2]" : "bg-[#7bd3eb]/[0.16] text-[#ccebf5]"}`}
                      >
                        {FIELD_TYPES.find(
                      ...
                      {field.fieldType === "formula" &&
                        field.formula?.parts && (
                          <span
                            className="bg-white/[0.04] text-[#dce8f2] px-2 py-0.5 rounded"
                            dir="ltr"
                          >
                            {field.formula.parts
                              .map(part => {

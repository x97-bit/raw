  ...
  return (
    <div className="surface-card rounded-xl shadow-sm border border-white/[0.06] overflow-hidden p-0">
      <div className="px-4 py-3 bg-white/[0.03] border-b border-white/[0.06] flex items-center justify-between">
        ...
        <button onClick={onHide} className="text-[#91a0ad] hover:text-[#dce8f2]">
          <X size={18} />
        </button>
      </div>
      <div className="p-4 space-y-4">
        <div>
          <label className="block text-sm font-medium text-[#c8d4df] mb-1">اسم الحقل *</label>
          <input
            type="text"
        ...
        <div>
          <label className="block text-sm font-medium text-[#c8d4df] mb-1">نوع الحقل</label>
          <select
        ...
        {hasMultipleTargets && (
          <div className="space-y-2">
            <div className="flex items-center justify-between gap-3">
              <label className="block text-sm font-medium text-[#c8d4df]">الشاشات المستهدفة</label>
              <span className="text-xs text-[#91a0ad]">يمكن تحديد أكثر من شاشة عند الحاجة</span>
            </div>
        ...
        {newField.fieldType === 'select' && (
          <div>
            <label className="block text-sm font-medium text-[#c8d4df] mb-1">الخيارات (مفصولة بفواصل)</label>
            <input
              type="text"
        ...
        {isFormula && (
          <div className="space-y-3">
            <label className="block text-sm font-medium text-[#c8d4df]">المعادلة الحسابية</label>
        ...
        {!isFormula && (
          <div>
            <label className="block text-sm font-medium text-[#c8d4df] mb-1">القيمة الافتراضية (اختياري)</label>
            <input
              type="text"
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <label className="block text-sm font-medium text-[#c8d4df]">الأقسام التي يظهر فيها هذا الحقل</label>
            <button onClick={onSelectAllSections} className="text-xs text-[#7bd3eb] hover:text-white">
              {newField.sections.length === compatibleSectionCount ? 'إلغاء الكل' : 'تحديد الكل'}
            </button>
          </div>
          {hasMultipleTargets && selectedTargets.length > 1 && (
            <p className="mb-2 text-xs text-[#91a0ad]">سيظهر هذا الحقل في الأقسام المتوافقة مع الشاشات المحددة.</p>
          )}
          <div className="flex flex-wrap gap-2">

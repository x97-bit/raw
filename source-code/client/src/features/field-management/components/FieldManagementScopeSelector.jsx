...
const ACTIVE_BUTTON_CLASS = 'bg-white/10 text-[#eef3f7] shadow-[0_14px_24px_rgba(0,0,0,0.16)] ring-1 ring-white/10';
const IDLE_BUTTON_CLASS = 'bg-white/6 text-[#91a0ad] hover:bg-white/10 hover:text-[#eef3f7]';
const ACTIVE_TARGET_CLASS = 'bg-[linear-gradient(180deg,rgba(24,31,40,0.98)_0%,rgba(31,41,52,0.98)_100%)] ring-1 ring-[#648ea9]/28 shadow-[0_18px_34px_rgba(0,0,0,0.24)]';
      ...
      <div className="space-y-5">
        <div>
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
            <label className="text-sm font-bold text-[#eef3f7]">1. اختر القسم</label>
          ...
          <div className="space-y-3">
            {Object.entries(sectionGroups).map(([groupLabel, groupedSections]) => (
              <div key={groupLabel}>
                <div className="mb-1.5 px-1 text-xs font-bold text-[#91a0ad]">{groupLabel}</div>
        ...
        <div className="border-t border-white/6 pt-4">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
            <div>
              <label className="text-sm font-bold text-[#eef3f7]">2. اختر الشاشة الفرعية</label>
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-3">
                    <div className={`rounded-2xl p-3 ${isActive ? 'bg-white/12 text-[#eef3f7]' : 'bg-white/8 text-[#91a0ad]'}`}>
                      <TargetIcon size={18} />
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between gap-2">
                        <span className={`text-sm font-bold ${isActive ? 'text-[#eef3f7]' : 'text-[#c8d4df]'}`}>
                          {target.label}
                        </span>
                        {isActive && (
                      ...
                      <p className="mt-1 text-xs leading-5 text-[#91a0ad]">{targetMeta.description}</p>
                    </div>
                  </div>
                </button>
              );

        ...
        <table className="w-full text-sm">
                  ...
                  {columns.map((column, index) => {
                    if (TRIAL_BALANCE_SUMMABLE_KEYS.includes(column.key)) {
                      const sum = rows.reduce(
                        (total, row) => total + (row[column.dataKey] || 0),
                        0
                      );
                      return (
                        <td
                          key={column.key}
                          className={`px-3 py-2 ${column.isCenter ? "text-center" : ""} ${column.colorFn ? column.colorFn(sum) : column.colorClass || ""}`}
                        >
                          {column.key === "shipment_count" ||
                          column.key === "transaction_count"
                            ? sum
                            : formatTrialBalanceNumber(sum)}
                        </td>
                      );
                    }
          ...
          <tfoot>
            <tr className="border-t border-white/[0.08] bg-white/[0.05] font-bold">
              {columns.map((column, index) => {
                if (
                  TRIAL_BALANCE_SUMMABLE_KEYS.includes(column.key) &&
                  data.totals[column.dataKey] !== undefined
                ) {
                  const value = data.totals[column.dataKey] || 0;
                  return (
                    <td
                      key={column.key}
                      className={`px-3 py-3 ${column.isCenter ? "text-center" : ""} ${column.colorFn ? column.colorFn(value) : column.colorClass || ""}`}
                    >
                      {column.key === "shipment_count" ||
                      column.key === "transaction_count"
                        ? value
                        : formatTrialBalanceNumber(value)}
                    </td>
                  );
                }

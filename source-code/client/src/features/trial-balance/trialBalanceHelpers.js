import {
  buildFieldConfigMap,
  getFieldLabel,
} from "../../utils/fieldConfigMetadata";
import {
...
export function buildTrialBalanceColumns(
  visibleColumns = [],
  fieldConfigMap = {}
) {
  return visibleColumns
    .map(key => {
      ...
      return column
        ? { ...column, label: getFieldLabel(fieldConfigMap, key, column.label) }
        : null;
    })
    .filter(Boolean);
}
...
export function buildTrialBalanceSummaryCards(totals = {}) {
  return [
    {
      label: "عدد الحسابات",
      value: totals.account_count || 0,
      valueClassName: "text-[#eef3f7]",
    },
    {
      label: "عدد الشحنات",
      value: totals.shipment_count || 0,
      valueClassName: "text-[#9ab6ca]",
    },
    {
      label: "إجمالي المدين ($)",
      value: `$${formatTrialBalanceNumber(Math.round(totals.debit_usd || 0))}`,
      valueClassName: POSITIVE_TONE,
    },
    {
      label: "إجمالي الدائن ($)",
      value: `$${formatTrialBalanceNumber(Math.round(totals.credit_usd || 0))}`,
      valueClassName: NEGATIVE_TONE,
    },
    {
      label: "صافي الرصيد ($)",
      value: `$${formatTrialBalanceNumber(Math.round(totals.balance_usd || 0))}`,
      valueClassName: getFinancialTone(totals.balance_usd || 0),
    },
    {
      label: "إجمالي الربح ($)",
      value: `$${formatTrialBalanceNumber(Math.round(totals.profit_usd || 0))}`,
      valueClassName: getFinancialTone(totals.profit_usd || 0),
    },
    {
      label: "صافي الرصيد (د.ع)",
      value: formatTrialBalanceNumber(Math.round(totals.balance_iqd || 0)),
      valueClassName: getFinancialTone(totals.balance_iqd || 0),
    },
  ];
}
...
export function buildTrialBalanceExportSummary(totals = {}) {
  return [
    ...
    {
      label: "مدين ($)",
      value: `$${formatTrialBalanceNumber(totals.debit_usd)}`,
    },
    {
      label: "دائن ($)",
      value: `$${formatTrialBalanceNumber(totals.credit_usd)}`,
    },
    {
      label: "صافي ($)",
      value: `$${formatTrialBalanceNumber(totals.balance_usd)}`,
    },
    {
      label: "الربح ($)",
      value: `$${formatTrialBalanceNumber(totals.profit_usd)}`,
    },
  ];
}

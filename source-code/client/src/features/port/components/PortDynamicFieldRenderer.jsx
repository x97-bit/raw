...
export default function PortDynamicFieldRenderer({
  item,
  values,
  onChange,
  formatValue,
      ...
      <div key={item.key}>
        ...
        <input
          ...
          onChange={event =>
            onChange(
              field.fieldKey,
              sanitizeCustomFieldValue(field, event.target.value)
            )
          }
          className="input-field"
        />
  ...
  if (item.kind === "formula") {
    ...
    return (
      ...
      >
        <span className="mb-1 block text-xs font-semibold text-utility-muted">
          {item.field.label}
        </span>
        <p

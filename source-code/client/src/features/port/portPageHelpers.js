...
export const isTransportSectionScope = ({
  sectionKey,
  portId,
  accountType,
} = {}) =>
  resolvePortSectionKey(portId, accountType) === "transport-1" ||
...
export const getPortBuiltInFieldLabel = (
  sectionKey,
  fieldKey,
  formType,
  fallbackLabel
...
function canFillBuiltInField(visibleBuiltInFieldKeys, fieldKey) {
  ...
  if (visibleBuiltInFieldKeys instanceof Set)
    return visibleBuiltInFieldKeys.has(fieldKey);
  return Array.isArray(visibleBuiltInFieldKeys)
    ? visibleBuiltInFieldKeys.includes(fieldKey)
    : false;
}
...
export const buildPortStatementQuery = ({
  portId,
  accountType,
  from,
  to,
...
export const dedupePortCustomFieldsById = (fieldGroups = []) =>
  (fieldGroups || [])
    ...
    .filter(
      (field, index, fields) =>
        fields.findIndex(item => item.id === field.id) === index
    );
...
export const buildPortColumnsForTarget = ({
  ...
  const builtInColumns = getSectionColumns(sectionKey, target)
    .map((column, index) => {
      const isRequiredStatementColumn =
        target === "statement" &&
        STATEMENT_CORE_COLUMN_KEYS.includes(column.key);
      return {
        ...column,
        label: getFieldLabel(configMap, column.key, column.label),
        sortOrder: configMap[column.key]?.sortOrder ?? index + 1,
        visible: isRequiredStatementColumn
          ? true
          : (configMap[column.key]?.visible ?? column.defaultVisible ?? true),
      };
    })
...
export const getVisiblePortCustomFieldsForTarget = ({
  ...
  (customFields || [])
    ...
    .sort(
      (left, right) =>
        (configMap[left.fieldKey]?.sortOrder || 999) -
        (configMap[right.fieldKey]?.sortOrder || 999)
    );
...
export const getVisiblePortFormulaFieldsForTarget = ({
  ...
  (customFields || [])
    .filter(
      field =>
        matchesFieldTarget(field, sectionKey, target) &&
        field.fieldType === "formula" &&
        configMap[field.fieldKey]?.visible === true
    )
    .map(field => ({
      ...field,
      label: getFieldLabel(configMap, field.fieldKey, field.label),
...
export const applySuggestedDefaultsToForm = ({
  ...
  if (canFillBuiltInField(visibleBuiltInFieldKeys, "currency"))
    fill("Currency", defaults.Currency);
  if (canFillBuiltInField(visibleBuiltInFieldKeys, "driver_name"))
    fill("DriverID", defaults.DriverID);
  if (canFillBuiltInField(visibleBuiltInFieldKeys, "vehicle_plate"))
    fill("VehicleID", defaults.VehicleID);
  if (canFillBuiltInField(visibleBuiltInFieldKeys, "good_type"))
    fill("GoodTypeID", defaults.GoodTypeID);
  if (canFillBuiltInField(visibleBuiltInFieldKeys, "gov_name"))
  ...
  if (canFillBuiltInField(visibleBuiltInFieldKeys, "carrier_name"))
    fill("CarrierID", defaults.CarrierID);
  if (canFillBuiltInField(visibleBuiltInFieldKeys, "cost_usd"))
    fill("CostUSD", defaults.CostUSD);
  if (canFillBuiltInField(visibleBuiltInFieldKeys, "amount_usd"))
    fill("AmountUSD", defaults.AmountUSD);
  if (canFillBuiltInField(visibleBuiltInFieldKeys, "cost_iqd"))
    fill("CostIQD", defaults.CostIQD);
  if (canFillBuiltInField(visibleBuiltInFieldKeys, "amount_iqd"))
    fill("AmountIQD", defaults.AmountIQD);
  if (canFillBuiltInField(visibleBuiltInFieldKeys, "fee_usd"))
    fill("FeeUSD", defaults.FeeUSD);
  if (canFillBuiltInField(visibleBuiltInFieldKeys, "syr_cus"))
    fill("SyrCus", defaults.SyrCus);
  if (canFillBuiltInField(visibleBuiltInFieldKeys, "car_qty"))
    fill("CarQty", defaults.CarQty);
  if (canFillBuiltInField(visibleBuiltInFieldKeys, "trans_price"))
...
export const getPortStatementFooterCell = (
  column,
  index,
  totals = {},
  options = {}
  ...
  if (
    isTransport &&
    (metricKey === "amount_usd" || metricKey === "amountusd")
  ) {
    return {
      value: "-",
  ...
  if (
    isTransport &&
    (metricKey === "amount_iqd" || metricKey === "amountiqd")
  ) {
    return {
  ...
  if (metricKey === "cost_usd" || metricKey === "costusd") {
    return {
      value: `$${formatPortNumber(totals.totalCostUSD || 0)}`,
      className: "px-3 py-3",
    };
  }
  if (metricKey === "cost_iqd" || metricKey === "costiqd") {
    return {
      value: formatPortNumber(totals.totalCostIQD || 0),
      className: "px-3 py-3",
    };
  }
  if (metricKey === "fee_usd" || metricKey === "feeusd") {
    return {
      value: `$${formatPortNumber(totals.totalFeeUSD || 0)}`,
      className: "px-3 py-3",
    };
  }

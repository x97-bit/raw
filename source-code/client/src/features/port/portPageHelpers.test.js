...
describe("portPageHelpers", () => {
  it("creates stable filter defaults and resolves section keys", () => {
    ...
    expect(createPortFilters(15)).toEqual({
      accountId: "15",
      from: "",
      to: "",
    });
  ...
  it("builds transaction, account, and statement queries safely", () => {
    expect(
      ...
      "port=3&accountType=2&accountId=11&startDate=2026-04-01&endDate=2026-04-09&search=test&limit=30&offset=60"
    );

    expect(buildPortAccountsQuery({ portId: 1, accountType: 5 })).toBe(
      "port=1&accountType=5"
    );
    expect(
      buildPortAccountsQuery({ portId: "transport-1", accountType: 2 })
    ).toBe("accountType=2");
    expect(
      buildPortStatementQuery({
        portId: 2,
  ...
  it("applies suggested defaults only to visible empty built-in fields", () => {
    const next = applySuggestedDefaultsToForm({
      ...
      visibleBuiltInFieldKeys: new Set([
        "currency",
        "driver_name",
        "amount_usd",
      ]),
  ...
  it("builds defaults payloads and transport-aware footer cells", () => {
    ...
    expect(
      getPortStatementFooterCell({ key: "ProfitUSD" }, 2, {
        totalProfitUSD: -50,
      })
    ).toEqual({
      value: "$-50",
  ...
  it("exposes transport-specific labels and relabels ref columns", () => {
    expect(
      getPortViewLabels({ sectionKey: "port-1", formType: 3 }).debitLabel
    ).toBe("سند إضافة");
    expect(
      getPortViewLabels({ sectionKey: "transport-1", formType: 1 }).invoiceLabel
    ).toBe("استحقاق نقل");
    expect(
      getPortViewLabels({ sectionKey: "transport-1", formType: 2 }).paymentLabel
    ).toBe("سند دفع");
    expect(getPortBuiltInFieldLabel("port-1", "ref_no", 3, "fallback")).toBe(
      "رقم سند الإضافة"
    );
    expect(
      getPortBuiltInFieldLabel("transport-1", "ref_no", 1, "fallback")
    ).toBe("رقم استحقاق النقل");
    expect(
      getPortBuiltInFieldLabel("transport-1", "ref_no", 2, "fallback")
    ).toBe("رقم سند الدفع");
    expect(
      relabelPortColumnsForSection(
        [{ key: "ref_no", label: "رقم الفاتورة" }],
        "transport-1"
      )
    ).toEqual([{ key: "ref_no", label: "رقم المستند" }]);
  });
  ...
  it("deduplicates custom fields and builds visible port columns", () => {
    ...
    expect(dedupePortCustomFieldsById([[customField], [customField]])).toEqual([
      customField,
    ]);
    ...
    expect(columns.some(column => column.key === "custom_weight_note")).toBe(
      true
    );
    expect(
      columns.find(column => column.key === "custom_weight_note")?.label
    ).toBe("Custom Note");
  });
  ...
  it("filters visible custom and formula fields by target and visibility", () => {
    ...
    const configMap = {
      custom_text: {
        visible: true,
        sortOrder: 2,
        displayLabel: "Visible Custom",
      },
      custom_formula: {
        visible: true,
        sortOrder: 1,
        displayLabel: "Visible Formula",
      },

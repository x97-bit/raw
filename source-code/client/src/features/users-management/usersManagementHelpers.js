...
export const generateTemporaryPassword = (length = 12) => {
  const safeLength = Math.max(PASSWORD_MIN_LENGTH, Math.min(32, Number(length) || 12));
  const requiredChars = [
  ...
  const password = shuffleCharacters([...requiredChars, ...remainingChars].join(''));
  return isStrongPassword(password) ? password : generateTemporaryPassword(safeLength);
};

...
describe("usersManagementHelpers", () => {
  ...
  it("validates required create-user fields and password strength", () => {
    expect(validateNewUserForm(createInitialUserForm())).toBe(
      "جميع الحقول مطلوبة"
    );
  ...
  it("toggles and normalizes permission selections", () => {
    expect(togglePermissionSelection(["reports"], "debts")).toEqual([
      "reports",
      "debts",
    ]);
    expect(togglePermissionSelection(["reports", "debts"], "reports")).toEqual([
      "debts",
    ]);
    expect(
      normalizePermissionList(["reports", "reports", "", null, "debts"])
    ).toEqual(["reports", "debts"]);

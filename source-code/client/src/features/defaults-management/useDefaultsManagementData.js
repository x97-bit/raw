...
export default function useDefaultsManagementData({
  ...
  const loadDefaults = useCallback(async () => {
    ...
    try {
      const [accountRows, routeRows] = await Promise.all([
        api(
          `/defaults/account?sectionKey=${encodeURIComponent(selectedSection)}`
        ),
        api(
          `/defaults/route?sectionKey=${encodeURIComponent(selectedSection)}`
        ),
      ]);
  ...
  const filteredAccountDefaults = useMemo(
    () =>
      filterDefaultsRows(accountDefaultsList, search, [
        "accountName",
        "defaultDriverName",
        "defaultGovName",
        "defaultCompanyName",
  ...
  const filteredRouteDefaults = useMemo(
    () =>
      filterDefaultsRows(routeDefaultsList, search, [
        "govName",
        "currency",
        "sectionKey",
      ]),
  ...
  const saveAccount = useCallback(async () => {
    ...
    try {
      await api("/defaults/account", {
        method: "POST",
        body: JSON.stringify(
          buildAccountDefaultsPayload(accountForm, selectedSection)
        ),
      });
  ...
  const saveRoute = useCallback(async () => {
    ...
    try {
      await api("/defaults/route", {
        method: "POST",
        body: JSON.stringify(
          buildRouteDefaultsPayload(routeForm, selectedSection)
        ),
      });

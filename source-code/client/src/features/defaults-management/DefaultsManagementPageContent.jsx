...
export default function DefaultsManagementPage({ onBack }) {
  ...
  const activeForm = isAccountTab
    ? defaultsState.accountForm
    : defaultsState.routeForm;
  const setActiveForm = isAccountTab
    ? defaultsState.setAccountForm
    : defaultsState.setRouteForm;
  const activeFieldDefs = isAccountTab ? accountFieldDefs : routeFieldDefs;
  const handleReset = isAccountTab
    ? defaultsState.resetAccountForm
    : defaultsState.resetRouteForm;
  const handleSave = isAccountTab
    ? defaultsState.saveAccount
    : defaultsState.saveRoute;

  const renderDefaultsRow = useCallback(
    row => {
      if (isAccountTab) {
        return (
          <DefaultsListRow

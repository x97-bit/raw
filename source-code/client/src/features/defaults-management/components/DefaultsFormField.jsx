...
export default function DefaultsFormField({ definition, form, setForm, lookups }) {
  if (definition.type === 'autocomplete') {
    return (
      <FieldBlock key={definition.formId} label={definition.label} className={definition.className}>
        <AutocompleteInput
          value={form[definition.formText]}
          options={lookups[definition.source]}
          labelKey={definition.labelKey}
          valueKey={definition.valueKey}
          onChange={(text) => setForm((current) => ({
            ...current,
    ...
    return (
      <FieldBlock key={definition.formId} label={definition.label} className={definition.className}>
        <select
          ...
          onChange={(event) => setForm((current) => ({ ...current, [definition.formId]: event.target.value }))}
          className="input-field"
        >
          {definition.options.map((option, index) => (
            <option key={`${option.value || 'empty'}-${option.label}-${index}`} value={option.value}>
              {option.label}
            </option>
          ))}
        </select>

...
export default function ReportsHaiderProfitsView({
  ...
  const summaryCards = useMemo(
    () => buildHaiderReportSummaryCards(totals),
    [totals]
  );
          ...
          data && (
            <>
              <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6">
                ...
                <SummaryCard
                  label="إجمالي الكلفة ($)"
                  value={`$${formatReportNumber(totals.totalCostUSD)}`}
                  tone="text-utility-muted"
                />
                <SummaryCard
                  label="إجمالي المبلغ ($)"
                  value={`$${formatReportNumber(totals.totalAmountUSD)}`}
                  tone="text-utility-strong"
                />
                <SummaryCard
                  label="إجمالي الربح ($)"
                  value={`$${formatReportNumber(totals.totalProfitUSD)}`}
                  tone={profitUsdTone}
                />
                <SummaryCard
                  label="إجمالي الربح (د.ع)"
                  value={formatReportNumber(totals.totalNetIQD)}
                  tone={netIqdTone}
                />
                <SummaryCard
                  label="إجمالي الفرق (د.ع)"
                  value={formatReportNumber(totals.totalDifferenceIQD)}
                  tone="text-utility-warning-text"
                />
              </div>
                ...
                <div className="flex items-center justify-between border-b border-utility-soft-border bg-utility-soft-bg px-5 py-3.5">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <span className="font-bold text-utility-warning-text">
                      الفرق (د.ع):{" "}
                      {formatReportNumber(totals.totalDifferenceIQD)}
                    </span>
                  </div>
                </div>
                  ...
                  <table className="w-full text-sm">
                      ...
                      {rows.length === 0 ? (
                        ...
                        rows.map((row, index) => (
                          ...
                          >
                            ...
                            <td className="px-4 py-3 text-utility-muted">
                              {formatReportNumber(row.CostIQD)}
                            </td>
                            <td className="px-4 py-3">
                              {formatReportNumber(row.AmountIQD)}
                            </td>
                            <td className="px-4 py-3 text-utility-warning-text">
                              {formatReportNumber(row.DifferenceIQD)}
                            </td>
                            <td
                              className={`px-4 py-3 font-bold ${getProfitTone(row.NetIQD)}`}
                            >
                              {formatReportNumber(row.NetIQD)}
                            </td>
                          </tr>
                        ))
                      )}
                    ...
                    <tfoot>
                      <tr className="border-t border-utility-panel-border bg-utility-soft-bg font-bold">
                        ...
                        <td className="px-4 py-3">
                          {formatReportNumber(totals.totalWeight)}
                        </td>
                        <td className="px-4 py-3 text-utility-muted">
                          ${formatReportNumber(totals.totalCostUSD)}
                        </td>
                        <td className="px-4 py-3">
                          ${formatReportNumber(totals.totalAmountUSD)}
                        </td>
                        <td className={`px-4 py-3 ${profitUsdTone}`}>
                          ${formatReportNumber(totals.totalProfitUSD)}
                        </td>
                        <td className="px-4 py-3 text-utility-muted">
                          {formatReportNumber(totals.totalCostIQD)}
                        </td>
                        <td className="px-4 py-3">
                          {formatReportNumber(totals.totalAmountIQD)}
                        </td>
                        <td className="px-4 py-3 text-utility-warning-text">
                          {formatReportNumber(totals.totalDifferenceIQD)}
                        </td>
                        <td className={`px-4 py-3 ${netIqdTone}`}>
                          {formatReportNumber(totals.totalNetIQD)}
                        </td>
                      </tr>
                    </tfoot>
                  </table>

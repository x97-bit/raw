...
export default function useTransactionFormLayout({
    ...
    () =>
      customFields.filter(
        field =>
          ...
          (field.placement || "transaction") === "transaction"
      ),
    [customFields]
  );
  ...
  const builtInFormFieldsByKey = useMemo(
    () =>
      Object.fromEntries(
        activeBuiltInFormFields.map(field => [field.key, field])
      ),
    [activeBuiltInFormFields]
  );
      ...
      activeFormLayout
        .map(section => ({
          ...
          fields: section.keys
            ...
            .filter(
              field => field && (fieldConfigMap[field.key]?.visible ?? true)
            ),
        }))
  ...
  const getBuiltInFieldLabel = useCallback(
    (fieldKey, fallbackLabel) =>
      getFieldLabel(
        fieldConfigMap,
        fieldKey,
        fallbackLabel || builtInFieldLabelMap[fieldKey] || fieldKey
      ),

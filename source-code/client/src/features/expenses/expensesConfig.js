...
export function getExpensePortLabel(value) {
  return (
    PORT_OPTIONS.find(option => option.value === value)?.label || value || "-"
  );
}
...
export function getExpenseChargeTargetLabel(value) {
  return (
    EXPENSE_TARGET_OPTIONS.find(option => option.value === value)?.label ||
    "على المنفذ"
  );
}
...
export const EXPENSE_COLUMNS = [
  ...
  },
  {
    ...
    label: "المبلغ ($)",
    render: value =>
      value && value !== "0" ? `$${formatExpenseNumber(value)}` : "-",
    bold: true,
  },
  {
    key: "amount_iqd",

...
export default function ExpensesOverviewCards({
  totals,
  summaryRows,
  filterPort,
  onTogglePort,
  ...
  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
      ...
      >
        ...
        <p className="text-xl font-bold text-[#9ab6ca]">
          ${formatExpenseNumber(totals.totalUSD || 0)}
        </p>
        {(totals.totalIQD || 0) !== 0 && (
          <p className="mt-1 text-sm text-[#c8d4df]">
            {formatExpenseNumber(totals.totalIQD)} د.ع
          </p>
        )}
        <p className="mt-1.5 text-xs text-[#91a0ad]">
          {totals.count || 0} مصروف
        </p>
      </div>
      ...
      {summaryRows.map(summary => (
        <div
          ...
          onClick={() =>
            onTogglePort(filterPort === summary.portId ? "" : summary.portId)
          }
        >
          <div className="mb-3 flex items-center gap-3">
          ...
          <p className="text-xl font-bold text-[#c8d4df]">
            ${formatExpenseNumber(summary.totalUSD)}
          </p>
          {summary.totalIQD !== 0 && (
            <p className="mt-1 text-sm text-[#a9b8c6]">
              {formatExpenseNumber(summary.totalIQD)} د.ع
            </p>
          )}

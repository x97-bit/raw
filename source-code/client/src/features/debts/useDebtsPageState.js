...
export default function useDebtsPageState({ api }) {
  ...
  const dateFilteredDebts = useMemo(() => (
    (data.debts || []).filter((debt) => filterDebtByDate(debt.TransDate, filters.from, filters.to))
  ), [data.debts, filters.from, filters.to]);

  const summaryRows = useMemo(() => buildDebtSummaryRows(dateFilteredDebts), [dateFilteredDebts]);
  const filteredDebts = useMemo(() => (
    dateFilteredDebts.filter((debt) => !filters.accountName || debt.AccountName === filters.accountName)
  ...
  const activeColumns = getDebtorConfig(activeAccountName)?.columns || DEFAULT_DEBT_COLUMNS;
  const formConfig = getDebtorConfig(accountText || form.AccountName);
  const formFields = [...(formConfig?.formFields || DEFAULT_DEBT_FORM_FIELDS), ...COMMON_DEBT_FORM_FIELDS];
  ...
  const exportColumns = useMemo(() => buildDebtExportColumns(activeColumns), [activeColumns]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage(editingDebt ? 'تم تحديث الدين بنجاح.' : 'تمت إضافة الدين بنجاح.');
      closeForm();
      await load();
    } catch (error) {
  ...
  const handleDelete = async (debt) => {
    if (!debt || !window.confirm(`هل تريد حذف الدين الخاص بـ ${debt.AccountName}؟`)) {
      return;
    }

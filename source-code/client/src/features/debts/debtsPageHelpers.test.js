...
describe('debtsPageHelpers', () => {
  ...
  it('builds merged account options from accounts and debts', () => {
    expect(buildDebtAccountOptions(
      [{ AccountID: 1, AccountName: 'باسم' }, { AccountID: 2, AccountName: 'نعمان' }],
  ...
  it('builds export columns and preview items from configured columns', () => {
    const previewColumns = [
      { key: 'trans_date', dataKey: 'TransDate', label: 'التاريخ', render: (value) => value || '-' },
      { key: 'notes', dataKey: 'Notes', label: 'ملاحظات', render: (value) => value || '' },
    ];
  ...
  it('builds totals cards with stable labels and tones', () => {
    expect(buildDebtTotalsCards({
      ...
      { key: 'total_usd', label: 'إجمالي الدولار', value: '$100', tone: 'text-[#9ab6ca]' },
      { key: 'total_iqd', label: 'إجمالي الدينار', value: '2,000', tone: 'text-[#eef3f7]' },
      { key: 'paid_usd', label: 'المسدد دولار', value: '$40', tone: 'text-[#8eb8ad]' },
      { key: 'remaining_usd', label: 'المتبقي دولار', value: '$60', tone: 'text-[#c697a1]' },
      { key: 'remaining_iqd', label: 'المتبقي دينار', value: '2,000', tone: 'text-[#d1b58b]' },
      { key: 'count', label: 'عدد الحركات', value: '3', tone: 'text-[#9aa8b7]' },
    ]);
  });
});

...
export function buildDebtPreviewItems(selectedDebt, previewColumns = []) {
  ...
  const items = [
    { key: 'account_name', label: 'اسم الحساب', value: selectedDebt.AccountName || '-' },
    {
      key: 'remaining_usd',
      label: 'المتبقي ($)',
      value: selectedDebt.RemainingUSD ? `$${formatDebtNumber(selectedDebt.RemainingUSD)}` : '$0',
    },
    {
      key: 'remaining_iqd',
      label: 'المتبقي (د.ع)',
      value: selectedDebt.RemainingIQD ? formatDebtNumber(selectedDebt.RemainingIQD) : '0',
    },
    {
      key: 'paid_usd',
      label: 'المسدد ($)',
      value: selectedDebt.PaidAmountUSD ? `$${formatDebtNumber(selectedDebt.PaidAmountUSD)}` : '$0',
    },
    {
      key: 'paid_iqd',
      label: 'المسدد (د.ع)',
      value: selectedDebt.PaidAmountIQD ? formatDebtNumber(selectedDebt.PaidAmountIQD) : '0',
    },
...
export function buildDebtTotalsCards(totals = {}) {
  return [
    { key: 'total_usd', label: 'إجمالي الدولار', value: `$${formatDebtNumber(totals.totalUSD)}`, tone: DEBT_CARD_TONES.usd },
    { key: 'total_iqd', label: 'إجمالي الدينار', value: formatDebtNumber(totals.totalIQD), tone: DEBT_CARD_TONES.iqd },
    { key: 'paid_usd', label: 'المسدد دولار', value: `$${formatDebtNumber(totals.paidUSD)}`, tone: DEBT_CARD_TONES.paid },
    { key: 'remaining_usd', label: 'المتبقي دولار', value: `$${formatDebtNumber(totals.remainingUSD)}`, tone: DEBT_CARD_TONES.remainingUsd },
    { key: 'remaining_iqd', label: 'المتبقي دينار', value: formatDebtNumber(totals.remainingIQD), tone: DEBT_CARD_TONES.remainingIqd },
    { key: 'count', label: 'عدد الحركات', value: formatDebtNumber(totals.count), tone: DEBT_CARD_TONES.muted },
  ];
}

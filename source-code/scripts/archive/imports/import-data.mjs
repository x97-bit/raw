...
async function main() {
  ...
  try {
    ...
    for (const name of allGoods.keys()) {
      await conn.query("INSERT INTO goods_types (id, name) VALUES (?, ?)", [
        goodId,
        name,
      ]);
    ...
    for (const c of cashStates) {
      await conn.query("INSERT INTO cash_state (id, state) VALUES (?, ?)", [
        c.Cash_ID,
        c.Cash_State,
      ]);
    }
    ...
    for (const t of ksaTraders) {
      const [result] = await conn.query(
        ...
        [
          accountAutoId,
          cleanStr(t.KSA_Trider_Name),
          "تاجر",
          "port-1",
          currencyStr(t.Currency),
        ]
      );
    ...
    for (const t of qaimTraders) {
      const [result] = await conn.query(
        ...
        [
          accountAutoId,
          cleanStr(t.QAIM_Trider_Name),
          "تاجر",
          "port-3",
          currencyStr(t.Currency),
          cleanStr(t.Merchant_Report),
        ]
      );
    ...
    async function getOrCreateDriver(name) {
      ...
      await conn.query("INSERT INTO drivers (id, name) VALUES (?, ?)", [
        driverAutoId,
        n,
      ]);
    ...
    async function getOrCreateVehicle(plate) {
      ...
      await conn.query("INSERT INTO vehicles (id, plateNumber) VALUES (?, ?)", [
        vehicleAutoId,
        p,
      ]);
    ...
    for (const t of shrTrans) {
      ...
      await conn.query(
        ...
        [
          cleanStr(t.Ref_No),
          t.Tran_Type_ID === 1 ? "IN" : "OUT",
          parseDate(t.Tran_Date),
          accountId,
          currencyStr(t.Currency_ID || 2),
          driverId,
          vehicleId,
          goodTypeId,
          num(t.Weight) || null,
    ...
    for (const d of haiderDebts) {
      await conn.query(
        ...
        [
          ...
          num(d.Amount_USD),
          num(d.Amount_IQD),
          cleanStr(d.Driver_Name),
          cleanStr(d.Car_Number),
          cleanStr(d.Good_Type),
          num(d.Weight) || null,
          num(d.Meters) || null,
          parseDate(d.Trans_Date),
          cleanStr(d.Trider_Note),
        ]
      );
    }
    ...
    for (const d of basimDebts) {
      await conn.query(
        ...
        [
          ...
          num(d.FEE_USD),
          num(d.FEE_IQD),
          cleanStr(d.Trans_Type),
          parseDate(d.D_B_Date),
          cleanStr(d.Notes),
          num(d.FX_RATE) || null,
          cleanStr(d.State),
        ]

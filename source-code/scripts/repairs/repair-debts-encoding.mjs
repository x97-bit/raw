...
function repairDescription(value) {
  ...
  const parts = String(value)
    ...
    .map(part => {
      ...
      const label =
        DESCRIPTION_LABELS[labelIndex] ||
        DESCRIPTION_LABELS[DESCRIPTION_LABELS.length - 1];
      labelIndex += 1;
...
async function main() {
  ...
  const connection = await mysql.createConnection(
    buildConnectionOptions(databaseUrl)
  );
  ...
  try {
    ...
    const backupPath = path.join(
      BACKUP_DIR,
      `debts-encoding-repair-${stamp}.json`
    );
    await fs.writeFile(
      backupPath,
      JSON.stringify(
        {
          ...
          rows: affectedRows.map(({ row, updates }) => ({
            before: row,
            updates,
          })),
        },

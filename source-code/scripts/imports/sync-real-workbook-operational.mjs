...
function simplifyAccountLookupKey(value) {
  ...
  return normalizeLookupKey(withoutParenthetical)
    ...
    .filter(
      token =>
        token &&
        !["شركه", "شركة", "قديم", "جديد", "دولار", "دينار"].includes(token)
    )
    .join(" ")
    .trim();
}
...
function inferDirection(recordType) {
  ...
  return outHints.some(hint => text.includes(normalizeLookupKey(hint)))
    ? "OUT"
    : "IN";
}
...
function resolveWorkbookPath() {
  ...
  const candidates = fs
    ...
    .map(name => ({
      ...
      mtime: fs.statSync(path.join(downloadsDir, name)).mtimeMs,
    }))
...
function mapQaimRows(rows) {
  return rows
    ...
    .map(row => ({
      ...
      currency: inferCurrencyFromAmounts(
        row.costUSD,
        row.amountUSD,
        row.costIQD,
        row.amountIQD
...
async function createDatabaseBackup(connection, workbookPath) {
  ...
  const outputPath = path.join(
    BACKUP_DIR,
    `pre-operational-sync-${buildTimestampStamp()}.json`
  );
...
async function ensureEntityAlias(
  ...
  entityId,
  aliasName,
  sourceSystem = "real-workbook-operational"
) {
...
async function loadDatabaseContext(connection) {
  const [
    ...
    connection.query(
      "SELECT id, name, accountType, portId, currency, merchantReport FROM accounts ORDER BY name"
    ),
...
function createSectionState(dbContext) {
  const state = {
    ...
    drivers: {
      ...
      map: createLookupMap(
        dbContext.drivers,
        dbContext.aliases.filter(row => row.entityType === "driver")
      ),
    },
    vehicles: {
      ...
      map: createLookupMap(
        dbContext.vehicles,
        dbContext.aliases.filter(row => row.entityType === "vehicle")
      ),
    },
    goods: {
      ...
      map: createLookupMap(
        dbContext.goods,
        dbContext.aliases.filter(row => row.entityType === "good_type")
      ),
    },
    governorates: {
      ...
      map: createLookupMap(
        ...
        dbContext.aliases.filter(row => row.entityType === "governorate")
      ),
    },
  };
  ...
  for (const spec of [
    ...SECTION_SPECS,
    { sectionKey: "port-3", accountType: "تاجر" },
  ]) {
    if (state.accountsBySection[spec.sectionKey]) continue;
    const items = dbContext.accounts.filter(
      row => row.portId === spec.sectionKey
    );
    const aliases = dbContext.aliases.filter(
      row =>
        row.entityType === "account" &&
        items.some(item => item.id === row.entityId)
    );
...
async function findOrCreateAccount(
  ...
  state,
  rawName,
  { sectionKey, accountType, currency = null, merchantReport = null }
) {
  ...
  const existing =
    sectionState.map.get(normalizeLookupKey(normalizedName)) ||
    sectionState.map.get(simplifyAccountLookupKey(normalizedName));
  if (existing) {
  ...
  if (simplified && !sectionState.map.has(simplified))
    sectionState.map.set(simplified, entity);
...
async function findOrCreateLookupEntity(
  ...
  if (entityType === "driver") {
    ...
    [result] = await connection.query(
      ...
      [normalizedName, extra.trancePrice ?? null]
    );
  } else {
  ...
  const entity = {
    ...
    ...(entityType === "governorate"
      ? { trancePrice: extra.trancePrice ?? null }
      : {}),
  };
...
function summarizeSectionRows(spec, rows, dbContext) {
  ...
  for (const row of rows) {
    const effectiveRef =
      normalizeSpaces(row.refNo) || getSyntheticRef(spec, row);
    const candidate = {
      ...row,
    ...
    if (
      (spec.kind !== "transport" && !candidate.traderName) ||
      (spec.kind === "transport" && !candidate.carrierName)
    ) {
      decisions.push({
        ...candidate,
        status: "blocked",
...
async function insertSectionRow(connection, state, spec, row, counters) {
  if (spec.kind === "qaim") {
    ...
    const driver = row.driverName
      ...
      : null;
    const vehicle = row.vehiclePlate
      ? await findOrCreateLookupEntity(
          connection,
          state.vehicles,
          "vehicle",
  ...
  if (spec.kind === "mnz") {
    const account = await findOrCreateAccount(
      ...
      state,
      row.traderName,
      { sectionKey: "port-2", accountType: "تاجر" }
    );
    const driver = row.driverName
      ? await findOrCreateLookupEntity(
          connection,
  ...
  if (spec.kind === "transport") {
    const carrierAccount = await findOrCreateAccount(
      connection,
      state,
      row.carrierName,
      { sectionKey: "transport-1", accountType: "نقل" }
    );
    const traderAccount = row.traderName
      ? await findOrCreateAccount(connection, state, row.traderName, {
          sectionKey: "port-3",
          accountType: "تاجر",
        })
      : null;
  ...
  try {
    ...
    const dryRunReport = {
      ...
      sections: sectionPlans.map(plan => ({
        ...
        firstReadyRows: plan.decisions
          .filter(row => row.status === "ready")
          .slice(0, 10),
        firstBlockedRows: plan.decisions
          .filter(row => row.status === "blocked")
          .slice(0, 10),
      })),
    };

    const dryRunReportPath = saveReport(
      "real-workbook-operational-dry-run",
      dryRunReport
    );
    console.log(`Workbook: ${workbookPath}`);
    for (const plan of sectionPlans) {
      console.log(
        `${plan.sectionKey}: total=${plan.totalRows}, ready=${plan.readyRows}, duplicate=${plan.duplicateRows}, blocked=${plan.blockedRows}`
      );
    }
    ...
    const totalReady = sectionPlans.reduce(
      (sum, plan) => sum + plan.readyRows,
      0
    );
    if (totalReady === 0) {
    ...
    const existingRefs = new Map(
      SECTION_SPECS.map(spec => [
        spec.sectionKey,
        getExistingRefSet(dbContext, spec.sectionKey),
      ])
    );
    const existingFingerprints = new Map(
      SECTION_SPECS.map(spec => [
        spec.sectionKey,
        getExistingFingerprintSet(dbContext, spec.sectionKey),
      ])
    );
    ...
    try {
      for (const plan of sectionPlans) {
        for (const row of plan.decisions.filter(
          entry => entry.status === "ready"
        )) {
          const sectionRefSet = existingRefs.get(plan.sectionKey);
          const sectionFingerprintSet = existingFingerprints.get(
            plan.sectionKey
          );
          ...
          const inserted = await insertSectionRow(
            connection,
            state,
            plan,
            row,

...
async function main() {
  ...
  try {
    ...
    const adminRows = beforeSnapshot.data.app_users.filter(
      row => row.username === "admin"
    );

    if (adminRows.length !== 1) {
      throw new Error(
        `Expected exactly one admin user before reset, found ${adminRows.length}`
      );
    }
    ...
    await connection.query(
      "UPDATE `app_users` SET `active` = 1, `role` = 'admin' WHERE `username` = 'admin'"
    );

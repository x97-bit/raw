...
const backup = require('./database/backups/pre-safe-reset-2026-04-07T06-05-09-712Z.json');
...
async function restore() {
  ...
  const order = [
    'app_users', 'ports', 'account_types', 'goods_types', 'governorates',
    'drivers', 'vehicles', 'companies', 'accounts', 'cash_state',
    'custom_fields', 'field_config', 'transactions', 'debts', 'expenses',
    'special_accounts', 'entity_aliases', 'account_defaults', 'route_defaults',
    'custom_field_values', 'payment_matching', 'audit_logs',
  ];
  ...
  for (const table of order) {
    ...
    try {
      ...
      for (const rawRow of rows) {
        ...
        const vals = cols.map(c => fixedRow[c] === undefined ? null : fixedRow[c]);

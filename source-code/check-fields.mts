...
async function check() {
  ...
  (rows as any[]).forEach((r: any) =>
    console.log(`  ${r.section_key} | ${r.field_key} | visible=${r.visible}`)
  );
  ...
  const [sections] = await c.execute(
    `SELECT DISTINCT section_key FROM field_config ORDER BY section_key`
  );

...
mysql -h %DB_HOST% -P %DB_PORT% -u %DB_USER% -p%DB_PASS% < "%SQL_DIR%\01_schema.sql"
if %ERRORLEVEL% neq 0 (
...
mysql -h %DB_HOST% -P %DB_PORT% -u %DB_USER% -p%DB_PASS% < "%SQL_DIR%\02_seed_data.sql"
if %ERRORLEVEL% neq 0 (
...
mysql -h %DB_HOST% -P %DB_PORT% -u %DB_USER% -p%DB_PASS% < "%SQL_DIR%\03_indexes.sql"
if %ERRORLEVEL% neq 0 (
...
if /i "%LOAD_SAMPLE%"=="y" (
    ...
    mysql -h %DB_HOST% -P %DB_PORT% -u %DB_USER% -p%DB_PASS% < "%SQL_DIR%\04_sample_data.sql"

...
describe("Duplicate Prevention Logic", () => {
  ...
  describe("Accounts (Traders)", () => {
    it("should return existing account when name matches", async () => {
      ...
      const existingAccount = { id: 5, name: "تاجر موجود", accountType: "1", portId: "1" };
      ...
      const found = existingAccount.name === data.name && 
      ...
      const response = found 
        ? { id: existingAccount.id, message: "التاجر موجود مسبقاً", existing: true }
    ...
    it("should allow same name in different ports", async () => {
      const existingAccount = { id: 1, name: "تاجر مشترك", accountType: "1", portId: "1" };
  ...
  describe("Drivers", () => {
    it("should return existing driver when name matches", () => {
      ...
      const response = found 
        ? { id: found.id, DriverID: found.id, DriverName: found.name, existing: true }
  ...
  describe("Vehicles", () => {
    it("should return existing vehicle when plate matches", () => {
      ...
      const response = found 
        ? { id: found.id, VehicleID: found.id, PlateNumber: found.plateNumber, existing: true }
  ...
  describe("Goods Types", () => {
    it("should return existing goods type when name matches", () => {
      ...
      const response = found 
        ? { id: found.id, GoodTypeID: found.id, TypeName: found.name, existing: true }
  ...
  describe("Frontend duplicate handling", () => {
    it("should show correct message when trader already exists", () => {
      const response = { id: 5, message: "التاجر موجود مسبقاً", existing: true };

...
describe("AutocompleteInput shared component", () => {
  ...
  it("should export a default function", () => {
    const content = fs.readFileSync(
      path.join(clientSrc, "components/AutocompleteInput.jsx"),
      "utf-8"
    );
...
describe("PortPage uses shared AutocompleteInput", () => {
  const portPageWrapperContent = fs.readFileSync(
    path.join(clientSrc, "pages/PortPage.jsx"),
    "utf-8"
  );
  ...
  it("should delegate built-in field rendering through the extracted port transaction form hook", () => {
    expect(portPageWrapperContent).toContain(
      "export { default } from '../features/port/PortPageContent';"
    );
    expect(portPageContent).toContain(
      "import usePortTransactionForm from './usePortTransactionForm'"
    );
    expect(portTransactionFormContent).toContain(
  ...
  it("should NOT have inline AutocompleteInput definition", () => {
    ...
    expect(portTransactionFormContent).not.toContain(
      "function AutocompleteInput("
    );
  });

  it("should use AutocompleteInput for trader field", () => {
    expect(builtInFieldContent).toContain(
  ...
  it("should pass labelKey and valueKey to all AutocompleteInput instances", () => {
    const autocompleteCount = (
      builtInFieldContent.match(/<AutocompleteInput/g) || []
    ).length;
    const labelKeyCount = (builtInFieldContent.match(/labelKey=/g) || [])
      .length;
    const valueKeyCount = (builtInFieldContent.match(/valueKey=/g) || [])
      .length;
...
describe("Debts feature uses shared AutocompleteInput", () => {
  const debtsWrapperContent = fs.readFileSync(
    path.join(clientSrc, "pages/DebtsPage.jsx"),
    "utf-8"
  );
  ...
  it("should delegate debt filters and form rendering through extracted debt components", () => {
    expect(debtsWrapperContent).toContain(
      "export { default } from '../features/debts/DebtsPageContent';"
    );
    expect(debtsContent).toContain(
  ...
  it("should use AutocompleteInput for the debt filters account field", () => {
    expect(filtersContent).toContain(
      "import AutocompleteInput from '../../../components/AutocompleteInput'"
    );
  ...
  it("should use AutocompleteInput for the debt form account field", () => {
    expect(formModalContent).toContain(
      "import AutocompleteInput from '../../../components/AutocompleteInput'"
    );
...
describe("TransactionModal uses shared AutocompleteInput", () => {
  const modalContent = fs.readFileSync(
    path.join(clientSrc, "components/TransactionModal.jsx"),
    "utf-8"
  );
  const editItemContent = fs.readFileSync(
    path.join(
      clientSrc,
      "features/transactions/components/TransactionModalEditItem.jsx"
    ),
    "utf-8"
  );
  ...
  it("should use shared AutocompleteInput for trader field in the extracted edit item", () => {
    expect(editItemContent).toContain(
      "import AutocompleteInput from '../../../components/AutocompleteInput'"
    );
    expect(editItemContent).toContain(
...
describe("Reports feature uses shared AutocompleteInput", () => {
  const reportsWrapperContent = fs.readFileSync(
    path.join(clientSrc, "pages/ReportsPage.jsx"),
    "utf-8"
  );
  ...
  it("should use AutocompleteInput for trader name field inside the extracted reports add trader view", () => {
    expect(addTraderContent).toContain(
      "import AutocompleteInput from '../../../components/AutocompleteInput'"
    );

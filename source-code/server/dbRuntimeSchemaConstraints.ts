...
export async function ensureRuntimeSchemaConstraints(connection: RuntimeSchemaConnection) {
  await ensureDateColumn(connection, "transactions", "trans_date", { nullable: false });
  ...
  await ensureIndex(connection, "transactions", "idx_good_type_id", "INDEX `idx_good_type_id` (`good_type_id`)");
  ...
  await ensureIndex(connection, "account_defaults", "idx_default_gov_id", "INDEX `idx_default_gov_id` (`default_gov_id`)");
  await ensureIndex(
    connection,
    "account_defaults",
  ...
  await ensureIndex(connection, "route_defaults", "idx_gov_id", "INDEX `idx_gov_id` (`gov_id`)");

  await ensureUniqueConstraint(connection, "transactions", "uk_transactions_ref_no", "UNIQUE (`ref_no`)");
  ...
  await ensureUniqueConstraint(connection, "account_defaults", "uk_account_defaults", "UNIQUE (`account_id`, `section_key`)");
  await ensureUniqueConstraint(
    connection,
    "route_defaults",
    "uk_route_defaults",

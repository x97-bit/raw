    ...
    async (req: AuthRequest, res: Response) => {
      try {
        ...
        const rows = await db
          ...
          .where(eq(specialAccounts.type, "haider"));
        const filters = getSpecialAccountDateFilters(
    ...
    async (req: AuthRequest, res: Response) => {
      try {
        ...
        const rows = await db
          .select()
          .from(specialAccounts)
          .where(inArray(specialAccounts.type, ["partnership", "yaser"]));
        const filters = getSpecialAccountDateFilters(
          req.query as Record<string, unknown>
        );
  ...
  router.put(
    ...
    async (req: AuthRequest, res: Response) => {
      try {
        ...
        const specialId = assertPositiveIntegerParam(
          req.params.id,
          SPECIAL_ACCOUNT_ID_LABEL
        );
        const [existingSpecial] = await db
          .select()
          .from(specialAccounts)
          .where(eq(specialAccounts.id, specialId))
          .limit(1);
        if (!existingSpecial)
          return res
            .status(404)
            .json({ error: SPECIAL_ACCOUNT_NOT_FOUND_MESSAGE });

        const payload = validateInput(
          specialAccountUpdateSchema,
          buildSpecialAccountMutationData(req.body, undefined, {
            partial: true,
          }) as Partial<SpecialAccountInsert>,
          UPDATE_VALIDATION_MESSAGE
        ) as Partial<SpecialAccountInsert>;
        if (Object.keys(payload).length === 0)
          return res.json({ message: NO_CHANGES_MESSAGE });
        ...
        await safeWriteAuditLog(db, {
          ...
          action: "update",
          summary:
            `${UPDATE_SUMMARY_PREFIX} ${existingSpecial.type || ""}`.trim(),
          before: existingSpecial,
          after: { ...existingSpecial, ...payload },
          appUser: req.appUser,
          metadata: {
            type: existingSpecial.type || null,
    ...
    async (req: AuthRequest, res: Response) => {
      try {
        ...
        await safeWriteAuditLog(db, {
          ...
          action: "delete",
          summary:
            `${DELETE_SUMMARY_PREFIX} ${existingSpecial.type || ""}`.trim(),
          before: existingSpecial,
          appUser: req.appUser,
          metadata: {
            type: existingSpecial.type || null,
            name: existingSpecial.name || null,
          },
        });

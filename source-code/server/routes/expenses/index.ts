  ...
  router.get(
    ...
    async (req: AuthRequest, res: Response) => {
      try {
        ...
        if (requestedPortId)
          conditions.push(eq(expenses.portId, requestedPortId));
  ...
  router.post(
    ...
    async (req: AuthRequest, res: Response) => {
      try {
        ...
        const chargeTarget = normalizeExpenseChargeTarget(
          req.body?.chargeTarget
        );
        ...
        await safeWriteAuditLog(db, {
          ...
          metadata: {
            portId: data.portId,
            chargeTarget: data.chargeTarget,
  ...
  router.put(
    ...
    async (req: AuthRequest, res: Response) => {
      try {
        ...
        const expenseId = assertPositiveIntegerParam(
          req.params.id,
          EXPENSE_ID_LABEL
        );
        const [existingExpense] = await db
          ...
          .limit(1);
        if (!existingExpense)
          return res.status(404).json({ error: EXPENSE_NOT_FOUND_MESSAGE });

        const nextChargeTarget =
          req.body?.chargeTarget !== undefined
            ? normalizeExpenseChargeTarget(req.body?.chargeTarget)
            : normalizeExpenseChargeTarget(existingExpense.chargeTarget);

        const resolvedAccount =
          nextChargeTarget === "trader"
            ? await resolveExpenseAccount(
                db,
                req.body?.accountId ?? existingExpense.accountId
              )
            : { accountId: null, accountName: null };

        const rawUpdates: Record<string, unknown> = {};
        if (req.body?.expenseDate !== undefined)
          rawUpdates.expenseDate = req.body.expenseDate;
        if (req.body?.amountUSD !== undefined)
          rawUpdates.amountUSD = String(req.body.amountUSD ?? "0");
        if (req.body?.amountIQD !== undefined)
          rawUpdates.amountIQD = String(req.body.amountIQD ?? "0");
        if (req.body?.description !== undefined)
          rawUpdates.description = req.body.description || null;
        if (req.body?.portId !== undefined)
          rawUpdates.portId = req.body.portId || "general";
        if (req.body?.chargeTarget !== undefined)
          rawUpdates.chargeTarget = nextChargeTarget;
        if (
        ...
        await safeWriteAuditLog(db, {
          ...
          appUser,
          metadata: {
            portId: updates.portId || existingExpense.portId,
            chargeTarget: updates.chargeTarget || existingExpense.chargeTarget,
          },
        });
    ...
    async (req: AuthRequest, res: Response) => {
      try {
        ...
        await safeWriteAuditLog(db, {
          ...
          appUser,
          metadata: {
            portId: existingExpense.portId || null,
            chargeTarget: existingExpense.chargeTarget || null,
          },
        });

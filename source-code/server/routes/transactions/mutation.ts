...
export function registerTransactionMutationRoutes(router: Router) {
  ...
  router.put("/transactions/:id", authMiddleware, financialWriteRateLimit, async (req: AuthRequest, res: Response) => {
    try {
      const db = await getDb();
      if (!db) return res.status(500).json({ error: "Database unavailable" });

      const transactionId = assertPositiveIntegerParam(req.params.id, TRANSACTION_ID_LABEL);
      const [existingTransaction] = await db.select().from(transactions).where(eq(transactions.id, transactionId)).limit(1);
      ...
      await safeWriteAuditLog(db, {
        ...
        appUser: req.appUser,
        metadata: { refNo: existingTransaction.refNo || null, changedKeys: Object.keys(validatedUpdates) },
      });

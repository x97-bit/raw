...
export async function buildTransactionUpdateInput(
  db: AppDb,
  ...
  if (hasBodyKey(body, "driverId", "DriverID", "driver_id")) {
    updates.driverId =
      parseOptionalInt(
        pickBodyField(body, "driverId", "DriverID", "driver_id")
      ) ?? null;
  }
  if (hasBodyKey(body, "vehicleId", "VehicleID", "vehicle_id")) {
    updates.vehicleId =
      parseOptionalInt(
        pickBodyField(body, "vehicleId", "VehicleID", "vehicle_id")
      ) ?? null;
  }
  if (hasBodyKey(body, "goodTypeId", "GoodTypeID", "good_type_id")) {
    updates.goodTypeId =
      parseOptionalInt(
        pickBodyField(body, "goodTypeId", "GoodTypeID", "good_type_id")
      ) ?? null;
  }
  if (
    hasBodyKey(
  ...
  const companyName = pickBodyField(
    body,
    "companyName",
    "CompanyName",
    "company_name"
  ...
  if (companyId) {
    const resolvedCompany = await resolveCompanySelection(
      db,
      companyId,
      companyName
    );
  ...
  if (hasBodyKey(body, "accountId", "AccountID", "account_id")) {
    updates.accountId = parseOptionalInt(
  ...
  return validateInput(
    ...
    updates,
    TRANSACTION_UPDATE_VALIDATION_MESSAGE
  );
}

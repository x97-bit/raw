...
export async function buildTransactionCreateInput(
  ...
  const recordType = String(
    pickBodyField(body, "recordType", "RecordType", "record_type") ?? "shipment"
  );
  const direction = getStoredDirectionValue(
    pickBodyField(body, "type", "TransTypeID", "direction")
  );
  ...
  const rawPortId = String(
    pickBodyField(body, "portId", "PortID", "port_id") ?? "GEN"
  );
  const prefix =
    rawPortId
      .toUpperCase()
      ...
      .substring(0, 5) || "GEN";
  const referenceCode =
    recordType.trim().toLowerCase() === "debit-note"
      ? "DBN"
  ...
  const inputDriverId =
    pickBodyField(body, "driverId", "DriverID", "driver_id") ?? null;
  const inputVehicleId =
    pickBodyField(body, "vehicleId", "VehicleID", "vehicle_id") ?? null;
  const inputGoodTypeId =
    pickBodyField(body, "goodTypeId", "GoodTypeID", "good_type_id") ?? null;
  ...
  const data = validateInput(
    transactionCreateSchema,
    {
      ...
      weight:
        parseOptionalDecimal(pickBodyField(body, "weight", "Weight")) ?? null,
      meters:
        parseOptionalDecimal(pickBodyField(body, "meters", "Meters")) ?? null,
      costUsd:
        parseOptionalDecimal(
          pickBodyField(body, "costUsd", "CostUSD", "cost_usd")
        ) ?? "0",
      amountUsd:
        parseOptionalDecimal(
          pickBodyField(body, "amountUsd", "AmountUSD", "amount_usd")
        ) ?? "0",
      costIqd:
        parseOptionalDecimal(
          pickBodyField(body, "costIqd", "CostIQD", "cost_iqd")
        ) ?? "0",
      amountIqd:
        parseOptionalDecimal(
          pickBodyField(body, "amountIqd", "AmountIQD", "amount_iqd")
        ) ?? "0",
      feeUsd:
        parseOptionalDecimal(
          pickBodyField(body, "feeUsd", "FeeUSD", "fee_usd")
        ) ?? "0",
      syrCus:
        parseOptionalDecimal(
          pickBodyField(body, "syrCus", "SyrCus", "syr_cus")
        ) ?? "0",
      carQty:
        parseOptionalInt(pickBodyField(body, "carQty", "CarQty", "car_qty")) ??
        null,
      transPrice:
        parseOptionalDecimal(
          pickBodyField(body, "transPrice", "TransPrice", "trans_price")
        ) ?? null,
      carrierId:
        parseOptionalInt(
          pickBodyField(body, "carrierId", "CarrierID", "carrier_id")
        ) ?? null,
      ...
      accountType: String(
        pickBodyField(body, "accountType", "AccountTypeID", "account_type") ??
          ""
      ),
      createdBy,
    },

...
import {
  requireDefaultAdmin,
  ROUTE_DEFAULTS_DELETED_MESSAGE,
  ROUTE_DEFAULTS_SAVED_MESSAGE,
} from "./shared";
...
function buildRouteDefaultPayload(
  ...
  return {
    ...
    govId,
    currency,
    defaultTransPrice: resolveDecimalValue(
      body.defaultTransPrice,
      existing?.defaultTransPrice,
      replaceMode
    ),
    ...
    defaultCostUsd: resolveDecimalValue(
      body.defaultCostUsd,
      existing?.defaultCostUsd,
      replaceMode
    ),
    defaultAmountUsd: resolveDecimalValue(
      body.defaultAmountUsd,
      existing?.defaultAmountUsd,
      replaceMode
    ),
    defaultCostIqd: resolveDecimalValue(
      body.defaultCostIqd,
      existing?.defaultCostIqd,
      replaceMode
    ),
    defaultAmountIqd: resolveDecimalValue(
      body.defaultAmountIqd,
      existing?.defaultAmountIqd,
      replaceMode
    ),
    ...
    async (req: AuthRequest, res: Response) => {
      try {
        ...
        if (!sectionKey || !govId) {
          return res
            .status(400)
            .json({ error: "sectionKey and govId are required" });
        }
        ...
        const payload = buildRouteDefaultPayload(
          req.body,
          existing,
          replaceMode,
          sectionKey,

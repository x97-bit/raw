...
import { mapChargedExpenseToStatementRow, normalizeExpenseChargeTarget } from "../../utils/expenses";
...
export function registerReportAccountStatementRoutes(router: Router) {
  router.get("/reports/account-statement/:id", authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (requestedEndDate) conditions.push(sql`${transactions.transDate} <= ${requestedEndDate}`);
      ...
      const chargedExpenseStatements = chargedExpenseRows
        ...
        .map((expense) => mapChargedExpenseToStatementRow(expense, account?.name || ""));
      const combinedRows = [...enrichedRows, ...chargedExpenseStatements].sort((left, right) => {
      ...
      const statementRows: AccountStatementRow[] = addRunningBalances(combinedRows).map((transaction) => ({
        ...transaction,

...
export function registerPaymentMatchingDashboardRoutes(router: Router) {
  router.get("/payment-matching/dashboard", authMiddleware, async (_req: AuthRequest, res: Response) => {
    try {
      ...
      const accountMap: Record<
        number,
        { account_id: number; AccountName: string; unpaid_count: number; remaining_usd: number; remaining_iqd: number }
      > = {};
      ...
      const topRemaining = Object.values(accountMap)
        .sort((left, right) => right.remaining_usd - left.remaining_usd || right.remaining_iqd - left.remaining_iqd)
        .slice(0, 20);

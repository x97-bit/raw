  ...
  router.delete(
    ...
    async (req: AuthRequest, res: Response) => {
      try {
        ...
        const allocationId = assertPositiveIntegerParam(
          req.params.id,
          ALLOCATION_ID_LABEL
        );
        await db
          .delete(paymentMatching)
          .where(eq(paymentMatching.id, allocationId));
      ...
      try {
        ...
        const [invoiceRows, paymentRows] = await Promise.all([
          db
            .select()
            .from(transactions)
            .where(eq(transactions.id, payload.invoiceId))
            .limit(1),
          db
            .select()
            .from(transactions)
            .where(eq(transactions.id, payload.paymentId))
            .limit(1),
        ]);
        ...
        if (!invoice || !payment) {
          return res
            .status(404)
            .json({ error: INVOICE_OR_PAYMENT_NOT_FOUND_MESSAGE });
        }
        if (!isInvoiceDirection(invoice.direction)) {
          return res
            .status(400)
            .json({ error: FIRST_RECORD_MUST_BE_INVOICE_MESSAGE });
        }
        if (!isPaymentDirection(payment.direction)) {
          return res
            .status(400)
            .json({ error: SECOND_RECORD_MUST_BE_PAYMENT_MESSAGE });
        }
        ...
        const existingMatchRows = await db
          ...
          .where(
            and(
              eq(paymentMatching.invoiceId, payload.invoiceId),
              eq(paymentMatching.paymentId, payload.paymentId)
            )
          )
          .limit(1);
        ...
        const invoiceRemainingUsd = Math.max(
          0,
          getAbsoluteAmount(invoice.amountUsd) -
            Number(invoiceUsageRows[0]?.amountUsd || 0)
        );
        const invoiceRemainingIqd = Math.max(
          0,
          getAbsoluteAmount(invoice.amountIqd) -
            Number(invoiceUsageRows[0]?.amountIqd || 0)
        );
        const paymentRemainingUsd = Math.max(
          0,
          getAbsoluteAmount(payment.amountUsd) -
            Number(paymentUsageRows[0]?.amountUsd || 0)
        );
        const paymentRemainingIqd = Math.max(
          0,
          getAbsoluteAmount(payment.amountIqd) -
            Number(paymentUsageRows[0]?.amountIqd || 0)
        );

        if (
          requestedUsd > invoiceRemainingUsd ||
          requestedUsd > paymentRemainingUsd
        ) {
          return res.status(400).json({ error: USD_OVERMATCH_MESSAGE });
        }
        if (
          requestedIqd > invoiceRemainingIqd ||
          requestedIqd > paymentRemainingIqd
        ) {
